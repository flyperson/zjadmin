    				...
    				<ul class="nav bs-docs-sidenav"> 					
    					...
    					<li>
    						...
    						<ul class="nav">
    							...
    							<li class="active"><a href="scsl6.php?gcid=<?php echo $row["id"];?>&yhid=<?php echo $row["用户id"];?>&zljcid=<?php echo $_GET["zljcid"];?>&sjc=<?php echo $_GET["sjc"];?>">木地板安装工程</a></li>
...
//										  $gcid1=$_GET["gcid"];
										...
										<!--<h3 class="panel-title label label-info"></h3>--><HR style="FILTER: progid:DXImageTransform.Microsoft.Shadow(color:#987cb9,direction:145,strength:15)" width="100%" color=#987cb9 SIZE=1>
												...
												<div class="form-group">
													...
													<div class="col-sm-3">
														...
														<input type="text" class="hidden" id="xmlb" name="xmlb" value="木地板安装">
									 ...
									 ?>
										<div class="form-group">						
											...
											<?php
											  require("../conn.php");
											  $zljcid=$_GET["zljcid"];
												$sql = "select * from 测点布置附件表  where 质量检查id='$zljcid' and 实测类别='实测实量' and 项目类别='木地板安装' ";
												$result = $conn->query($sql);
												while($row = $result->fetch_assoc()) {
                      ?>
        ...
        <button type="button" class="btn btn-default"  onclick="javascript:window.open('cdbz/scbz6.php?id=<?php echo $_GET["zljcid"];?>&xmlb=<?php echo '木地板安装' ?>&sclb=<?php echo '实测实量' ?>', 'newwindow2', 'height=470, width=1100, top=100, left=200,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')">
        	测点布置                                                                                                                                                                                                                                   
        </button>
		...
		<a href="#" id="myTabDrop1" class="dropdown-toggle" 
		   data-toggle="dropdown">测点类别筛选 <b class="caret"></b>
									...
									<ul class="dropdown-menu" role="menu" aria-labelledby="myTabDrop1" >
										...
										<li class=""><a href="#cd1"  tabindex="-1" data-toggle="tab">找平层平整度</a></li>
										<li class=""><a href="#cd2"  tabindex="-1" data-toggle="tab">木地板平整度</a></li>
			...
			<form id="cdbcform" name="cdbcform"  method="post" class="form-horizontal" role="form" >    							
                   ...
                   require("../conn.php");  
				   				 ...
				   				 $zljcid=$_GET["zljcid"]; 
								   	...
								   	$sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='木地板安装' ";
            ...
            <div class="form-group" id="ceshiyong">
                   ...
                   require("../conn.php");  
				   				 error_reporting(E_ALL ^ E_NOTICE);	 
				   				 $zljcid=$_GET["zljcid"]; 
								   $sql1 = "select * from  装修工程质量检查表  where  检查项目='木地板安装' and 检查内容='$xmlB' ";
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='木地板安装' and 测点类别='找平层平整度' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   $xmlB = $row["测点类别"];
                   ?>
            <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='木地板安装' and 测点类别='木地板平整度' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   $xmlB = $row["测点类别"];
                   ?>
...
//							   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='混凝土结构工程' ";
             $sql = "select 测点实测值 from 实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='木地板安装' and 测点类别 = '找平层平整度' order by CONVERT(测点实测值,SIGNED) desc ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
					...
					<div class="form-group col-sm-12" >
						...
						<div class="col-sm-2">
							<input type="text" class="form-control" id="cdlbA" name="cdlbA" value="找平层平整度" >
						</div>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
//							   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='混凝土结构工程' ";
             $sql = "select 测点实测值 from 实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='木地板安装' and 测点类别 = '木地板平整度' order by CONVERT(测点实测值,SIGNED) desc ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
					...
					<div class="form-group">
						<label for="cdlb" class="col-sm-3 control-label">测点类别：</label>
								<div class="col-sm-3">
				                   ...
				                   require("../conn.php");   
								   				 $zljcid=$_GET["zljcid"];                
				                   $sql = "select distinct 检查内容 from  装修工程质量检查表  where 检查项目='木地板安装'";
				                   $result = $conn->query($sql);
				                   while($row = $result->fetch_assoc()) {
				                ?>
						...
						<div class="form-group ">
							...
							<div class="col-sm-3">
								<input type="text" class="form-control hidden" id="xmlb" name="xmlb" value="木地板安装" placeholder="">
...
<script type="text/javascript">
 $("#cdlb").click(function(){ 
			 	...
			 	if(cdlb=="木地板平整度"){
			 		document.getElementById("nr").value='同一户选取2个房间（客厅及任意一个卧室）（1） 每个房间的地面作为1个实测区，累计实测实量10 个实测区。（2） 任选同一功能房间地面的2个对角区域，按与墙面夹角45度平放靠尺测量2次，加上房间中部区域测量一次，共测量3次。客、餐厅或较大房间地面的中部区域需加测1次。（3）同一功能房间内的3或4个平整度实测值，作为判断该实测指标合格率的3或4个计算点。';
			 		$("#cdsl").val("30");
      ...
      var s = 0;
			for(var i=0;i<30;i++){
				if(cdsczpl[i].value<3 && cdsczpl[i].value>0){
					s=s+1;
				}
   	};
			var jgdy = s/30*100;
...
//			************墙面垂直度********************//
      ...
      var s1 = 0;
			for(var i=0;i<30;i++){
				if(qmpzd[i].value<2 && qmpzd[i].value>0){
					s1=s1+1;
				}
				else{
					s1 =s1;
