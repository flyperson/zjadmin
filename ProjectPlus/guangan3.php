...
//										echo $_GET["id"];
    				...
    				<ul class="nav bs-docs-sidenav"> 					
    					...
    					<li><b style="margin-left:20px;">观感</b>
    						...
    						<ul class="nav">
    							...
    							<li class="active"><a href="guangan3.php?gcid=<?php echo $row["id"];?>&yhid=<?php echo $row["用户id"];?>&zljcid=<?php echo $_GET["zljcid"];?>&sjc=<?php echo $_GET["sjc"];?>">内墙抹灰工程</a></li>
	...
	<div class="panel-heading">
		<h3 class="panel-title">内墙抹灰工程</h3>
	</div>
...
//										  $gcid1=$_GET["gcid"];
										...
										<!--<h3 class="panel-title label label-info"></h3>--><HR style="FILTER: progid:DXImageTransform.Microsoft.Shadow(color:#987cb9,direction:145,strength:15)" width="100%" color=#987cb9 SIZE=1>
												...
												<div class="form-group">
													...
													<div class="col-sm-3">
														...
														<input type="text" class="hidden" id="xmlb" name="xmlb" value="内墙抹灰工程">
									 ...
									 ?>
										<div class="form-group">						
											...
											<?php
											  require("../conn.php");
											  $zljcid=$_GET["zljcid"];
												$sql = "select * from 测点布置附件表  where 质量检查id='$zljcid' and 实测类别='观感评价' and 项目类别='内墙抹灰工程'  ";
												$result = $conn->query($sql);
												while($row = $result->fetch_assoc()) {
                      ?>
        ...
        <button type="button" class="btn btn-default"  onclick="javascript:window.open('cdbz/ggbz3.php?id=<?php echo $_GET["zljcid"];?>&xmlb=<?php echo '内墙抹灰工程'?>&sclb=<?php echo '观感评价'?>', 'newwindow2', 'height=470, width=1100, top=100, left=200,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')">
        	测点布置
        </button>
			</div>
				<p></p>
		...
		<a href="#" id="myTabDrop1" class="dropdown-toggle" 
		   data-toggle="dropdown">测点类别筛选 <b class="caret"></b>
									...
									<ul class="dropdown-menu" role="menu" aria-labelledby="myTabDrop1" >
										...
										<li class=""><a href="#cd3"  tabindex="-1" data-toggle="tab">严重空鼓、开裂</a></li>
										</ul>
									<li>
									</ul>
			...
			<form id="cdbcform" name="cdbcform"  method="post" class="form-horizontal" role="form" >    							
                   ...
                   require("../conn.php");  
				   				 ...
				   				 $zljcid=$_GET["zljcid"]; 
								   	 ...
								   	 $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='内墙抹灰工程' ";
             ...
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='内墙抹灰工程' and 测点类别='工艺要求' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='内墙抹灰工程' and 测点类别='观感质量' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='内墙抹灰工程' and 测点类别='严重空鼓、开裂' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
              <div class="form-group" id="ceshiyong">
			...
			<div role="tabpanel" class="tab-pane fade " id="tjfx">
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='内墙抹灰工程' and 测点类别 = '工艺要求' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='内墙抹灰工程' and 测点类别 = '观感质量' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='内墙抹灰工程' and 测点类别 = '严重空鼓、开裂' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
					...
					<div class="form-group col-sm-12" >
						<label for="cdlbA" class="col-sm-2 control-label">测点类别：</label>
						<div class="col-sm-2">
							<input type="text" class="form-control" id="cdlbA" name="cdlbA" value="严重空鼓、开裂" >
						</div>
						<label for="hgl" class="col-sm-2 control-label">合格率：</label>
						<div class="col-sm-2">
...
（不足10个以实计算）</textarea>	
							...
							<div class="col-sm-3">
											<input type="text" class="form-control hidden" id="xmlb" name="xmlb" value="内墙抹灰工程" placeholder="">
...
//				alert('12300');
			...
			$("#cdlx1").click(function(){ 
			 	...
			 	if(cdlx1=="基底、拉毛"){
			 		document.getElementById("nr").value='拉毛、挂网前基底已按要求处理，洞口、线槽等已按要求修补，水电布管已完成；粘结牢固、毛刺感强，拉毛密度满足要求';
			 		$("#cdsl").val("10");
			 	}
			 	else if	(cdlx1=="加强网"){
			 		document.getElementById("nr").value='抹灰面层按两防要求挂玻纤网，搭接位置正确；门窗及洞口预留搭接网；不同材料交接处、线管剔槽修补处等按要求挂钢丝网';
			 		$("#cdsl").val("10");
			 ...
			 $("#cdlx2").click(function(){ 
			 	...
			 	if(cdlx2=="空鼓"){
			 		...
			 		$("#nr").val('表面洁净，色泽基本一致，接槎平整，不粗糙；修补规范，禁止未使用机械切割直接剔凿修补');
			 		$("#cdsl").val("10");
			 		
			 	}
			 		else if	(cdlx2=="细部外露、收口"){
			 		$("#nr").val('无钢丝网、玻纤网、钢筋头、混凝土面外露；孔洞、槽、盒等细部抹灰尺寸正确，边缘整齐；门窗洞口的抹灰层须压门窗框3~5mm（窗边铺贴砖或石材的除外）');
			 		$("#cdsl").val("10");
			...
			$("#cdlx3").click(function(){
				var cdlx3=$("#cdlx3").val();
				if(cdlx3=="单户通长裂缝"){
					$("#nr").val('（长度≥50cm）条数＞3，超出部分每条扣0.3分');
					$("#cdsl").val("10");
				}
				else if(cdlx3=='单户空鼓'){
					$("#nr").val('（20cm×20cm）或龟裂处数＞3，超出部分每处扣0.1分（以上每项每户扣分不超过2分）');
					$("#cdsl").val("10");
				}
			});
</script>
