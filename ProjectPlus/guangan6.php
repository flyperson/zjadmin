...
//										echo $_GET["id"];
    				...
    				<ul class="nav bs-docs-sidenav"> 					
    					...
    					<li><b style="margin-left:20px;">观感</b>
    						...
    						<ul class="nav">
    							...
    							<li class="active"><a href="guangan6.php?gcid=<?php echo $row["id"];?>&yhid=<?php echo $row["用户id"];?>&zljcid=<?php echo $_GET["zljcid"];?>&sjc=<?php echo $_GET["sjc"];?>">卫生间、沉箱工程</a></li>
	...
	<div class="panel-heading">
		<h3 class="panel-title">卫生间、沉箱工程</h3>
	</div>
...
//										  $gcid1=$_GET["gcid"];
										...
										<!--<h3 class="panel-title label label-info"></h3>--><HR style="FILTER: progid:DXImageTransform.Microsoft.Shadow(color:#987cb9,direction:145,strength:15)" width="100%" color=#987cb9 SIZE=1>
												...
												<div class="form-group">
													...
													<div class="col-sm-3">
														...
														<input type="text" class="hidden" id="xmlb" name="xmlb" value="卫生间、沉箱工程">
									 ...
									 ?>
										<div class="form-group">						
											...
											<?php
											  require("../conn.php");
											  $zljcid=$_GET["zljcid"];
												$sql = "select * from 测点布置附件表  where 质量检查id='$zljcid' and 实测类别='观感评价' and 项目类别='卫生间、沉箱工程'  ";
												$result = $conn->query($sql);
												while($row = $result->fetch_assoc()) {
                      ?>
        ...
        <button type="button" class="btn btn-default"  onclick="javascript:window.open('cdbz/ggbz6.php?id=<?php echo $_GET["zljcid"];?>&xmlb=<?php echo '卫生间、沉箱工程'?>&sclb=<?php echo '观感评价'?>', 'newwindow2', 'height=470, width=1100, top=100, left=200,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')">
        	测点布置
        </button>
			</div>
				<p></p>
		...
		<a href="#" id="myTabDrop1" class="dropdown-toggle" 
		   data-toggle="dropdown">测点类别筛选 <b class="caret"></b>
			...
			<form id="cdbcform" name="cdbcform"  method="post" class="form-horizontal" role="form" >    							
                   ...
                   require("../conn.php");  
				   				 ...
				   				 $zljcid=$_GET["zljcid"]; 
								   	 ...
								   	 $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='卫生间、沉箱工程' ";
              ...
              <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='卫生间、沉箱工程' and 测点类别='工艺要求' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='卫生间、沉箱工程' and 测点类别='观感质量' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='卫生间、沉箱工程' and 测点类别='材料要求及普遍渗漏' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
             <div class="form-group" id="ceshiyong">
			...
			<div role="tabpanel" class="tab-pane fade " id="tjfx">
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='卫生间、沉箱工程' and 测点类别 = '工艺要求' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='卫生间、沉箱工程' and 测点类别 = '观感质量' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='卫生间、沉箱工程' and 测点类别 = '材料要求及普遍渗漏' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
...
（不足10个以实计算）</textarea>	
							...
							<div class="col-sm-3">
											<input type="text" class="form-control hidden" id="xmlb" name="xmlb" value="卫生间、沉箱工程" placeholder="">
...
//				alert('12300');
			 ...
			 $("#cdlx1").click(function(){ 
			 	...
			 	if(cdlx1=="基底"){
			 		document.getElementById("nr").value='卫生间沉箱无使用铁线或套管对穿支模的情况；防水层施工前找坡符合要求，基底处理到位';
			 		$("#cdsl").val("10");
			 		
			 	}
			 	else if	(cdlx1=="防水"){
			 		document.getElementById("nr").value='防水层厚度、防水搭接、收口位置及方式符合要求、无起鼓';
			 		$("#cdsl").val("10");
			 		
			 	}
			 	else if	(cdlx1=="细部"){
			 		document.getElementById("nr").value='沉箱、阳露台阴角部位做泛水、防水层按要求上翻、细部防水收口到位及各排水管道安装工序正确；管道边防水附加层满足要求';
			 		$("#cdsl").val("10");
			 		
			 	}
			 	else if	(cdlx1=="同层排水"){
			 		document.getElementById("nr").value='施工工序正确，工艺符合要求；排水正常无堵塞';
			 		$("#cdsl").val("10");
			 ...
			 $("#cdlx2").click(function(){ 
			 	...
			 	var cdlx2=$("#cdlx2").val();
			 	if(cdlx2=="渗漏"){
			 		$("#nr").val('结构阶段、防水层施工后无渗漏现象');
			 		$("#cdsl").val("10");
			 		
			 	}
			 	else if	(cdlx2=="空鼓开裂"){
			 		$("#nr").val('找平层、防水层、保护层无开裂、空鼓、破损、强度不足等现象');
			 		$("#cdsl").val("10");
			 		
			 	}
			 		else if	(cdlx2=="降板处理"){
			 		$("#nr").val('卫生间、阳露台降板等满足设计要求（≥2CM）');
			 		$("#cdsl").val("10");
