    				...
    				<ul class="nav bs-docs-sidenav"> 					
    					...
    					<li>
    						...
    						<ul class="nav">
    							...
    							<li class="active"><a href="guangan17.php?gcid=<?php echo $row["id"];?>&yhid=<?php echo $row["用户id"];?>&zljcid=<?php echo $_GET["zljcid"];?>&sjc=<?php echo $_GET["sjc"];?>">电气工程</a></li>
...
//										  $gcid1=$_GET["gcid"];
										...
										<!--<h3 class="panel-title label label-info"></h3>--><HR style="FILTER: progid:DXImageTransform.Microsoft.Shadow(color:#987cb9,direction:145,strength:15)" width="100%" color=#987cb9 SIZE=1>
												...
												<div class="form-group">
													...
													<div class="col-sm-3">
														...
														<input type="text" class="hidden" id="xmlb" name="xmlb" value="电气工程">
									 ...
									 ?>
										<div class="form-group">						
											...
											<?php
											  require("../conn.php");
											  $zljcid=$_GET["zljcid"];
												$sql = "select * from 测点布置附件表  where 质量检查id='$zljcid' and 实测类别='观感评价' and 项目类别='电气工程'  ";
												$result = $conn->query($sql);
												while($row = $result->fetch_assoc()) {
                      ?>
        ...
        <button type="button" class="btn btn-default"  onclick="javascript:window.open('cdbz/ggbz17.php?id=<?php echo $_GET["zljcid"];?>&xmlb=<?php echo '电气工程'?>&sclb=<?php echo '观感评价'?>', 'newwindow2', 'height=420, width=315, top=0, left=500,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')">
        	测点布置
        </button>
			</div>
				<p></p>
		...
		<a href="#" id="myTabDrop1" class="dropdown-toggle" 
		   data-toggle="dropdown">测点类别筛选 <b class="caret"></b>
									...
									<ul class="dropdown-menu" role="menu" aria-labelledby="myTabDrop1" >
										...
										<li class=""><a href="#cd1"  tabindex="-1" data-toggle="tab">空调 </a></li>
										<li class=""><a href="#cd2"  tabindex="-1" data-toggle="tab">电箱、排气扇、开关、插座</a></li>
										<li class=""><a href="#cd3"  tabindex="-1" data-toggle="tab">灯具 </a></li>
									</ul>	
									<li>
									</ul>
			...
			<form id="cdbcform" name="cdbcform"  method="post" class="form-horizontal" role="form" >    							
                   ...
                   require("../conn.php");  
				   				 ...
				   				 $zljcid=$_GET["zljcid"]; 
								   	 ...
								   	 $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='电气工程' ";
             ...
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='电气工程' and 测点类别='空调' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='电气工程' and 测点类别='电箱、排气扇、开关、插座' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='电气工程' and 测点类别='灯具' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
              <div class="form-group" id="ceshiyong">
			...
			<div role="tabpanel" class="tab-pane fade " id="tjfx">
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='电气工程' and 测点类别 = '空调' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='电气工程' and 测点类别 = '电箱、排气扇、开关、插座  ' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='电气工程' and 测点类别 = '灯具' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
					...
					<div class="form-group col-sm-12" >
						...
						<div class="col-sm-2">
							<input type="text" class="form-control" id="cdlbA" name="cdlbA" value="空调" >
						</div>
					...
					<div class="form-group col-sm-12" >
						...
						<div class="col-sm-2">
							<input type="text" class="form-control" id="qbmpzd" name="qbmpzd" value="电箱、排气扇、开关、插座  " >
						</div>
					...
					<div class="form-group col-sm-12" >
						<label for="cdlbA" class="col-sm-2 control-label">测点类别：</label>
						<div class="col-sm-2">
							<input type="text" class="form-control" id="cdlbA" name="cdlbA" value="灯具" >
						</div>
						<label for="hgl" class="col-sm-2 control-label">合格率：</label>
						<div class="col-sm-2">
			...
			<div class="modal fade" id="myModal1" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
					...
					<div class="form-group">
						...
						<label for="cdlx1" class="col-sm-3 control-label">测点类型：</label>
										<div class="col-sm-3">
											<select id="cdlx1" name="cdlx1" class="form-control">
							                   ...
							                   require("../conn.php");  
							   				  $zljcid=$_GET["zljcid"]; 
											   $sql = "select 测点类型 from  观感评价测点类型表 where  工程类别='电气工程'and 测点类别='空调'  ";
							                   $result = $conn->query($sql);
								                   while($row = $result->fetch_assoc()) {
								                   ?>
											...
											<select id="cdlx2" name="cdlx2" class="form-control hidden">
							                   ...
							                   require("../conn.php");  
							   				  $zljcid=$_GET["zljcid"]; 
											   $sql = "select 测点类型 from  观感评价测点类型表 where  工程类别='电气工程'and 测点类别='电箱、排气扇、开关、插座'  ";
							                   $result = $conn->query($sql);
								                   while($row = $result->fetch_assoc()) {
								                   ?>
												<option><?php echo $row["测点类型"];?></option>
											...
											<select id="cdlx3" name="cdlx3" class="form-control hidden">
							                   ...
							                   require("../conn.php");  
							   				  $zljcid=$_GET["zljcid"]; 
											   $sql = "select 测点类型 from  观感评价测点类型表 where  工程类别='电气工程'and 测点类别='灯具'  ";
							                   $result = $conn->query($sql);
								                   while($row = $result->fetch_assoc()) {
								                   ?>
												<option><?php echo $row["测点类型"];?></option>
						...
						<div class="form-group ">	
								...
								<div class="col-sm-9">
									<textarea class="form-control" rows="6" id="cq"  name="cq">抽检10户中的30个功能间作为30个测区（不足30个以实计算）</textarea>	
								</div>	
							</div>
									<div class="form-group ">
							...
							<div class="col-sm-3">
											<input type="text" class="form-control hidden" id="xmlb" name="xmlb" value="电气工程" placeholder="">
	      ...
	      $('#cdlx1').addClass("hidden");
		    $('#cdlx2').addClass("hidden");	
		     $('#cdlx3').addClass("hidden");
				...
				else if (cdlb.value=='电箱、排气扇、开关、插座')
				{ 				
					$('#cdlx2').removeClass("hidden");			
				}
				else if (cdlb.value=='灯具')
				{ 				
				...
				$("#cdlx1").click(function(){ 
			 	...
			 	if(cdlx1=="孔洞"){
			 		document.getElementById("nr").value='位置、坡度正确，收口严密、美观'
			 		
			 	}
			 	else if	(cdlx1=="中央空调"){
			 		document.getElementById("nr").value='风口收口严密、平直、无污染、位置合理，安装牢固'
			 		
			 	}
			 	
			 	 	else if	(cdlx1=="分立式空调"){
			 		document.getElementById("nr").value='室内机位置合理，冷凝水管孔洞不外露（因结构限制，不能隐蔽，外露冷凝管不超10cm,孔洞外露的需有管套盖），插座与空调机侧面水平，外露电线小于20cm '
			 ...
			 $("#cdlx2").click(function(){ 
			 	...
			 	var cdlx2=$("#cdlx2").val();
			 	if(cdlx2=="箱内"){
			 		$("#nr").val('开关标识清晰；电线预留长度充足；穿线分色，中间不能有接头')
			 		
			 	}
			 	else if	(cdlx2=="配电箱、排气扇"){
			 		$("#nr").val('安装平直、牢固，收口严密，位置合理，外表面无明显刮痕，无污染')
			 		
			 	}
			 	else if(cdlx2=="开关、插座"){
			 		$("#nr").val('安装平直、牢固，标高正确，收口严密，位置合理，外表面无明显刮痕，无污染，厨卫插座设置防水盖')
			 	}
			...
			$("#cdlx3").click(function(){
				var cdlx3=$("#cdlx3").val();
				if(cdlx3=="高度"){
					$("#nr").val('符合安全要求；固定灯具的吊钩安装符合强度要求 ')
				}
				else if(cdlx3=="灯具"){
					$("#nr").val('安装位置合理，安装牢固，亮度合适，灯光效果符合设计要求，基座收口美观，排列整齐，无破损、划伤、污染等现象，周边无缝隙，收口美观')
				}
