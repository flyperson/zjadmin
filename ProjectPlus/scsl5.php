    				...
    				<ul class="nav bs-docs-sidenav"> 					
    					...
    					<li>
    						...
    						<ul class="nav">
    							...
    							<li class="active"><a href="scsl5.php?gcid=<?php echo $row["id"];?>&yhid=<?php echo $row["用户id"];?>&zljcid=<?php echo $_GET["zljcid"];?>&sjc=<?php echo $_GET["sjc"];?>">墙面饰面砖工程</a></li>
...
//										  $gcid1=$_GET["gcid"];
										...
										<!--<h3 class="panel-title label label-info"></h3>--><HR style="FILTER: progid:DXImageTransform.Microsoft.Shadow(color:#987cb9,direction:145,strength:15)" width="100%" color=#987cb9 SIZE=1>
												...
												<div class="form-group">
													...
													<div class="col-sm-3">
														...
														<input type="text" class="hidden" id="xmlb" name="xmlb" value="墙面饰面砖">
									 ...
									 ?>
										<div class="form-group">						
											...
											<?php
											  require("../conn.php");
											  $zljcid=$_GET["zljcid"];
												$sql = "select * from 测点布置附件表  where 质量检查id='$zljcid' and 实测类别='实测实量' and 项目类别='墙面饰面砖' ";
												$result = $conn->query($sql);
												while($row = $result->fetch_assoc()) {
                      ?>
        ...
        <button type="button" class="btn btn-default"  onclick="javascript:window.open('cdbz/scbz5.php?id=<?php echo $_GET["zljcid"];?>&xmlb=<?php echo '墙面饰面砖' ?>&sclb=<?php echo '实测实量' ?>', 'newwindow2', 'height=470, width=1100, top=100, left=200,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')">
        	测点布置                                                                                                                                                                                                                                   
        </button>
		...
		<a href="#" id="myTabDrop1" class="dropdown-toggle" 
		   data-toggle="dropdown">测点类别筛选 <b class="caret"></b>
									...
									<ul class="dropdown-menu" role="menu" aria-labelledby="myTabDrop1" >
										...
										<li class=""><a href="#cd1"  tabindex="-1" data-toggle="tab">垂直度</a></li>
										<li class=""><a href="#cd2"  tabindex="-1" data-toggle="tab">阴阳角</a></li>
										<li class=""><a href="#cd3"  tabindex="-1" data-toggle="tab">接缝高低差</a></li>
			...
			<form id="cdbcform" name="cdbcform"  method="post" class="form-horizontal" role="form" >    							
                   ...
                   require("../conn.php");  
				   				 ...
				   				 $zljcid=$_GET["zljcid"]; 
								   	$sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='墙面饰面砖' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   $xmlB = $row["测点类别"];
                   ?>
            <div class="form-group" id="ceshiyong">
                   ...
                   require("../conn.php");  
				   				 error_reporting(E_ALL ^ E_NOTICE);	 
				   				 $zljcid=$_GET["zljcid"]; 
								   $sql1 = "select * from  装修工程质量检查表  where  检查项目='墙面饰面砖' and 检查内容='$xmlB' ";
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='墙面饰面砖' and 测点类别  LIKE '%垂直度%' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   $xmlB = $row["测点类别"];
                   ?>
            <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='墙面饰面砖' and 测点类别  LIKE '%阴阳角%' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                  $xmlB = $row["测点类别"];
                   ?>
            <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='墙面饰面砖' and 测点类别  LIKE '%接缝高低差%' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   $xmlB = $row["测点类别"];
                   ?>
...
//							   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='混凝土结构工程' ";
             $sql = "select 测点实测值 from 实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='墙面饰面砖' and 测点类别 = '垂直度' order by CONVERT(测点实测值,SIGNED) desc ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
					...
					<div class="form-group col-sm-12" >
						...
						<div class="col-sm-2">
							<input type="text" class="form-control" id="cdlbA" name="cdlbA" value="垂直度" >
						</div>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
//							   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='混凝土结构工程' ";
             $sql = "select 测点实测值 from 实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='墙面饰面砖' and 测点类别 = '阴阳角' order by CONVERT(测点实测值,SIGNED) desc ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
					...
					<?php
	           require("../conn.php");  
             $sql = "select 测点实测值 from 实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='墙面饰面砖' and 测点类别 = '接缝高低差' order by CONVERT(测点实测值,SIGNED) desc ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
					...
					<div class="form-group col-sm-12" >
						...
						<div class="col-sm-2">
							<input type="text" class="form-control" id="czdmm" name="czdmm" value="接缝高低差" >
						</div>
					...
					<div class="form-group">
						<label for="cdlb" class="col-sm-3 control-label">测点类别：</label>
								<div class="col-sm-3">
				                   ...
				                   require("../conn.php");   
								   				 $zljcid=$_GET["zljcid"];                
				                   $sql = "select distinct 检查内容 from  装修工程质量检查表  where 检查项目='墙面饰面砖'";
				                   $result = $conn->query($sql);
				                   while($row = $result->fetch_assoc()) {
				                ?>
						...
						<div class="form-group">
							...
							<div class="col-sm-3">
								<input type="text" class="form-control hidden" id="xmlb" name="xmlb" value="墙面饰面砖" placeholder="">
...
<script type="text/javascript">
 $("#cdlb").click(function(){ 
			 	...
			 	if(cdlb=="垂直度(瓷砖墙面)"){
			 		document.getElementById("nr").value='同一户选取2个墙面（厨房及任一卫生间）（1）每一面墙都作为1个实测区，累计实测实量10 个实测区。如该户房内还有石材或木质墙面，则各增加5个实测区；（2）同一面墙距两端头竖向阴阳角约30cm位置，分别按以下原则实测2次：一是靠尺顶端接触到上部砼顶板位置时测1次垂直度，二是靠尺底端接触到下部地面位置时测1次垂直度；（3）以上各实测值作为合格率1个计算点。';
			 		$("#cdsl").val("20");
			 	...
			 	else if	(cdlb=="阴阳角(瓷砖墙面)"){
			 		document.getElementById("nr").value='同一户选取2个阴阳角（厨房及任一卫生间）（1）每户选取对观感影响较大的1个阴角及1个阳角进行测量，每一个阴角或阳角作为1个实测区，累计实测实量10 个测区；如该户房内还有石材或木质墙面，则各增加10个实测区；（2）同一个部位，从地面向上300mm和1500mm位置分别测量1次。测量时采用50cm角尺进行测量，以角尺短边靠紧墙面，用塞尺测量角尺长边与墙面缝隙最大处得出一个实测值。2次实测值作为判断该实测指标合格率的1个计算点。';
			 		$("#cdsl").val("20");
			 		
			 	}
			 	else if	(cdlb=="阴阳角(石材墙面)"){
			 		document.getElementById("nr").value='同一户选取2个阴阳角（厨房及任一卫生间）（1）每户选取对观感影响较大的1个阴角及1个阳角进行测量，每一个阴角或阳角作为1个实测区，累计实测实量10 个测区；如该户房内还有石材或木质墙面，则各增加10个实测区；（2）同一个部位，从地面向上300mm和1500mm位置分别测量1次。测量时采用50cm角尺进行测量，以角尺短边靠紧墙面，用塞尺测量角尺长边与墙面缝隙最大处得出一个实测值。2次实测值作为判断该实测指标合格率的1个计算点。';
			 		$("#cdsl").val("20");
			 		
			 	}
			 	else if	(cdlb=="阴阳角(木质墙面)"){
			 		document.getElementById("nr").value='同一户选取2个阴阳角（厨房及任一卫生间）（1）每户选取对观感影响较大的1个阴角及1个阳角进行测量，每一个阴角或阳角作为1个实测区，累计实测实量10 个测区；如该户房内还有石材或木质墙面，则各增加10个实测区；（2）同一个部位，从地面向上300mm和1500mm位置分别测量1次。测量时采用50cm角尺进行测量，以角尺短边靠紧墙面，用塞尺测量角尺长边与墙面缝隙最大处得出一个实测值。2次实测值作为判断该实测指标合格率的1个计算点。';
			 		$("#cdsl").val("20");
			 	}
			 	else if	(cdlb=="接缝高低差(瓷砖墙面)"){
			 		document.getElementById("nr").value='同一户选取2个墙面（厨房及任一卫生间）（1）该指标宜在装修收尾阶段测量。每一墙面作为1个实测区，累计实测实量10 个测区。如该户房内还有石材或木质墙面，则各增加3个实测区；（2） 在每一饰面砖墙面，目测选取2 条疑似高低差最大的饰面砖接缝。用钢尺或其他辅助工具紧靠相邻两饰面砖跨过接缝，用0.5mm 钢塞片插入钢尺与饰面砖之间的缝隙。如能插入，则该测量点不合格；反之则该测量点合格。2 条接缝高低差的实测值，分别作为判断该实测指标合格率的2 个计算点；（3） 为数据统计方便和提高实测效率，不合格点均按0.7mm 记录，合格点均按0.3mm记录。';
			 		$("#cdsl").val("20");
			 		
			 		
			 	}
			 	 	else if	(cdlb=="接缝高低差(石材墙面)"){
			 		document.getElementById("nr").value='同一户选取2个墙面（厨房及任一卫生间）（1）该指标宜在装修收尾阶段测量。每一墙面作为1个实测区，累计实测实量10 个测区。如该户房内还有石材或木质墙面，则各增加3个实测区；（2） 在每一饰面砖墙面，目测选取2 条疑似高低差最大的饰面砖接缝。用钢尺或其他辅助工具紧靠相邻两饰面砖跨过接缝，用0.5mm 钢塞片插入钢尺与饰面砖之间的缝隙。如能插入，则该测量点不合格；反之则该测量点合格。2 条接缝高低差的实测值，分别作为判断该实测指标合格率的2 个计算点；（3） 为数据统计方便和提高实测效率，不合格点均按0.7mm 记录，合格点均按0.3mm记录。';
			 		$("#cdsl").val("6");
			 		
			 		
			 	}
			 	 	else if	(cdlb=="接缝高低差(木材墙面)"){
			 		document.getElementById("nr").value='同一户选取2个墙面（厨房及任一卫生间）（1）该指标宜在装修收尾阶段测量。每一墙面作为1个实测区，累计实测实量10 个测区。如该户房内还有石材或木质墙面，则各增加3个实测区；（2） 在每一饰面砖墙面，目测选取2 条疑似高低差最大的饰面砖接缝。用钢尺或其他辅助工具紧靠相邻两饰面砖跨过接缝，用0.5mm 钢塞片插入钢尺与饰面砖之间的缝隙。如能插入，则该测量点不合格；反之则该测量点合格。2 条接缝高低差的实测值，分别作为判断该实测指标合格率的2 个计算点；（3） 为数据统计方便和提高实测效率，不合格点均按0.7mm 记录，合格点均按0.3mm记录。';
			 		$("#cdsl").val("6");
...
//			************墙面垂直度********************//
      ...
      var s1 = 0;
			for(var i=0;i<10;i++){
				if(qmpzd[i].value<3 && qmpzd[i].value>0){
					s1=s1+1;
				}
				else{
					s1 =s1;
...
//			************阴阳角方正********************//
      ...
      var s2 = 0;
			for(var i=0;i<20;i++){
				if(czdmmh[i].value<0.5 && czdmmh[i].value>0){
					s2=s2+1;
				}
   	};
			var jgdy2 = s2/20*100;
