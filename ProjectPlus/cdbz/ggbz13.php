		...
		if($cdlbaa=='普遍空鼓、开裂' ){
			?>
				...
				<?php
					if($cdlbaa=='涂料工程' ){
			?>
				<div class="win btn-xs btn-default " onclick="cdaniu(this.id);" name="<?php echo $row["测点编号"];?>&<?php echo $id;?>" id="<?php echo $row["测点编号"];?>&<?php echo $id;?>"  data-toggle="tooltip" data-placement="right" title="<?php echo $row["测点类别"];?>,<?php echo $row["测点实测值"];?>" style="position:fixed;left:<?php echo $xzzb;?>;top:<?php echo $yzzb;?>;background:#FFFFFF">
					<?php echo $row["测点编号"];?>
				</div>
				<?php }?>	
					<?php
					if($cdlbaa=='裱糊工程' ){
			?>
				<div class="win btn-xs btn-default " onclick="cdaniu(this.id);" name="<?php echo $row["测点编号"];?>&<?php echo $id;?>" id="<?php echo $row["测点编号"];?>&<?php echo $id;?>"  data-toggle="tooltip" data-placement="right" title="<?php echo $row["测点类别"];?>,<?php echo $row["测点实测值"];?>" style="position:fixed;left:<?php echo $xzzb;?>;top:<?php echo $yzzb;?>;background:#FF8800;">
					<?php echo $row["测点编号"];?>
				</div>
				<?php }?>	
					<?php
					if($cdlbaa=='木饰面' ){
			?>
				<div class="win btn-xs btn-default " onclick="cdaniu(this.id);" name="<?php echo $row["测点编号"];?>&<?php echo $id;?>" id="<?php echo $row["测点编号"];?>&<?php echo $id;?>"  data-toggle="tooltip" data-placement="right" title="<?php echo $row["测点类别"];?>,<?php echo $row["测点实测值"];?>" style="position:fixed;left:<?php echo $xzzb;?>;top:<?php echo $yzzb;?>;background:#00BBFF;">
					<?php echo $row["测点编号"];?>
				</div>
				<?php }?>	
					<?php
		  ...
		  <input class="hidden" id="sclb" value="<?php echo $sclb;?>"/>
			...
			<div style="position:fixed;left:350px;top:0px">
				<div class="row col-md-12"  style="height:400px;overflow:auto;">
					...
					<form method="post" class="form-horizontal col-md-2">	
						<input type="text" id="sxlba1" name="sxlba" class="hidden" value="普遍空鼓、开裂" />
						<button type="submit"  class="btn btn-warning">普遍空鼓、开裂</button>	
					</form>		
					<form method="post" class="form-horizontal col-md-2">
						<input type="text" id="sxlba2" name="sxlba" class="hidden" value="涂料工程" />	
						<button type="submit"  class="btn btn-default">涂料工程</button>	
					</form>	
					<form method="post" class="form-horizontal col-md-2">
						<input type="text" id="sxlba3" name="sxlba" class="hidden" value="裱糊工程" />	
						<button type="submit"  class="btn" style="background-color: #FF8800;">裱糊工程</button>	
					</form>	
					<form method="post" class="form-horizontal col-md-2">
						<input type="text" id="sxlba4" name="sxlba" class="hidden" value="木饰面" />	
						<button type="submit"  class="btn btn-info">木饰面</button>	
					</form>	
					<form method="post" class="form-horizontal col-md-2">
						<input type="text" id="sxlba5" name="sxlba" class="hidden" value="软硬包" />	
						<button type="submit"  class="btn btn-danger">软硬包</button>	
					</form>	
					<form method="post" class="form-horizontal col-md-2">
						<input type="text" id="sxlba6" name="sxlba" class="hidden" value="石材、瓷砖铺贴" />	
						<button type="submit"  class="btn btn-success">石材、瓷砖铺贴</button>	
					</form>	
				<form method="post" class="form-horizontal col-md-2">
						<input type="text" id="sxlba7" name="sxlba" class="hidden" value="玻璃、金属" />	
						<button type="submit"  class="btn btn-primary">玻璃、金属</button>	
					</form>	
				</div>
...
////					   echo $sxlba;
					   ...
					   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='墙面工程' ";
						 	...
						 	$sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='墙面工程' and 测点类别  LIKE '%$sxlba%' ";
						 }
					   $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	        ?>
