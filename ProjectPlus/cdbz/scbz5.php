...
//				echo $sclb;
			...
			?>
				...
				<?php }?>
				<?php
					if($cdlbaa=='接缝高低差(瓷砖墙面)' || $cdlbaa=='接缝高低差(石材墙面)' ||$cdlbaa=='接缝高低差(木质墙面)'  ){
			?>
				...
				<?php }?>	
					<?php
						if($cdlbaa=="垂直度(瓷砖墙面)"||$cdlbaa=="垂直度(石材墙面)"||$cdlbaa=="垂直度(木质墙面)"){
							?>
			...
			<div style="position:fixed;left:350px;top:0px">
				<div class="row col-md-12">
					...
					<form method="post" class="form-horizontal col-md-3">	
						<input type="text" id="sxlba1" name="sxlba" class="hidden" value="阴阳角" />
						<button type="submit"  class="btn btn-warning">阴阳角</button>	
					</form>		
					<form method="post" class="form-horizontal col-md-2">
					...
					<form method="post" class="form-horizontal col-md-3">	
						<input type="text" id="sxlba3" name="sxlba" class="hidden" value="接缝高低差" />	
						<button type="submit"  class="btn btn-info">接缝高低差</button>
					</form>	
...
////					   echo $sxlba;
					   ...
					   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='墙面饰面砖' ";
						 	...
						 	$sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='墙面饰面砖' and 测点类别  LIKE '%$sxlba%' ";
						 }
					   $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	        ?>
