...
//				echo $sclb;
			...
			?>
				...
				<?php }?>
				<?php
					if($cdlbaa=='墙面平整度(1)'|| $cdlbaa=='墙面平整度(2)'){
			?>
				...
				<?php }?>	
					<?php
						if($cdlbaa=="墙面垂直度(1)" ||$cdlbaa=="墙面垂直度(2)"){
							?>
					...
					<?php echo $row["测点编号"];?>
						...
						<?php } ?>
							<?php
						if($cdlbaa=="顶板水平度(1)" ||$cdlbaa=="顶板水平度(2)"){
							?>
			...
			<div style="position:fixed;left:350px;top:0px">
				<div class="row col-md-12">
					...
					<form method="post" class="form-horizontal col-md-3">	
						<input type="text" id="sxlba1" name="sxlba" class="hidden" value="开间/进深极差" />
						<button type="submit"  class="btn btn-warning">开间/进深极差</button>	
					</form>		
					<form method="post" class="form-horizontal col-md-2">
						<input type="text" id="sxlba2" name="sxlba" class="hidden" value="墙面平整度" />	
						<button type="submit"  class="btn btn-primary">墙面平整度</button>	
					</form>	
					<form method="post" class="form-horizontal col-md-3">	
						<input type="text" id="sxlba3" name="sxlba" class="hidden" value="墙面垂直度" />	
						<button type="submit"  class="btn btn-info">墙面垂直度</button>
					</form>	
					<form method="post" class="form-horizontal col-md-2">	
						<input type="text" id="sxlba4" name="sxlba" class="hidden" value="阴阳角" />	
						<button type="submit"  class="btn btn-danger">阴阳角</button>
					</form>	
					<form method="post" class="form-horizontal col-md-2">		
						<input type="text" id="sxlba5" name="sxlba" class="hidden" value="顶板水平度" />
						<button type="submit"  class="btn btn-success">顶板水平度</button>
					</form>		
				</div>
				<div class="col-md-12">
					<?php
...
////					   echo $sxlba;
					   ...
					   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='墙面涂饰面' ";
						 	...
						 	$sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='墙面涂饰面' and 测点类别  LIKE '%$sxlba%' ";
						 }
					   $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	        ?>
