...
//				echo $sclb;
			...
			?>
				...
				<?php }?>	
					...
					if($cdlbaa=='淋浴屏风' ){
			?>
				<div class="win btn-xs btn-default " onclick="cdaniu(this.id);" name="<?php echo $row["测点编号"];?>&<?php echo $id;?>" id="<?php echo $row["测点编号"];?>&<?php echo $id;?>"  data-toggle="tooltip" data-placement="right" title="<?php echo $row["测点类别"];?>,<?php echo $row["测点实测值"];?>" style="position:fixed;left:<?php echo $xzzb;?>;top:<?php echo $yzzb;?>;background:#00FFCC;">
					<?php echo $row["测点编号"];?>
				</div>
				<?php }?>	
					<?php
			...
			?>
				...
				<?php }?>	
					...
					if($cdlbaa=='给、排水' ){
			?>
				<div class="win btn-xs btn-default " onclick="cdaniu(this.id);" name="<?php echo $row["测点编号"];?>&<?php echo $id;?>" id="<?php echo $row["测点编号"];?>&<?php echo $id;?>"  data-toggle="tooltip" data-placement="right" title="<?php echo $row["测点类别"];?>,<?php echo $row["测点实测值"];?>" style="position:fixed;left:<?php echo $xzzb;?>;top:<?php echo $yzzb;?>;background:#00FF00;">
					<?php echo $row["测点编号"];?>
				</div>
				<?php }?>
		  ...
		  <input class="hidden" id="sclb" value="<?php echo $sclb;?>"/>
			...
			<div style="position:fixed;left:350px;top:0px">
				...
				<form method="post" class="form-horizontal col-md-3">	
						<input type="text" id="sxlba1" name="sxlba" class="hidden" value="马桶、浴缸、尿斗" />
						<button type="submit"  class="btn btn-warning">马桶、浴缸、尿斗</button>	
					</form>		
					<form method="post" class="form-horizontal col-md-2">
						<input type="text" id="sxlba2" name="sxlba" class="hidden" value="浴室镜、柜" />	
						<button type="submit"  class="btn btn-primary">浴室镜、柜</button>	
					</form>	
					<form method="post" class="form-horizontal col-md-3">	
						<input type="text" id="sxlba3" name="sxlba" class="hidden" value="淋浴屏风" />	
						<button type="submit"  class="btn btn-info">淋浴屏风</button>
					</form>	
					<form method="post" class="form-horizontal col-md-2">	
						<input type="text" id="sxlba4" name="sxlba" class="hidden" value="地漏" />	
						<button type="submit"  class="btn btn-danger">地漏</button>
					</form>	
					<form method="post" class="form-horizontal col-md-2">		
						<input type="text" id="sxlba5" name="sxlba" class="hidden" value="龙头、花洒、五金件" />
						<button type="submit"  class="btn btn-success">龙头、花洒、五金件</button>
					</form>		
					<form method="post" class="form-horizontal col-md-3">	
						<input type="text" id="sxlba3" name="sxlba" class="hidden" value="给、排水" />	
						<button type="submit"  class="btn" style="background-color: #BBFF00;">给、排水</button>
					</form>	
...
////					   echo $sxlba;
					   ...
					   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='卫生间、阳露台' ";
						 	...
						 	$sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='卫生间、阳露台' and 测点类别  LIKE '%$sxlba%' ";
						 }
					   $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	        ?>
