		...
		if($cdlbaa=='有吊顶天花'){
			...
			?>
				...
				<?php }?>	
					...
					if($cdlbaa=='线条' ){
			?>
				<div class="win btn-xs btn-default " onclick="cdaniu(this.id);" name="<?php echo $row["测点编号"];?>&<?php echo $id;?>" id="<?php echo $row["测点编号"];?>&<?php echo $id;?>"  data-toggle="tooltip" data-placement="right" title="<?php echo $row["测点类别"];?>,<?php echo $row["测点实测值"];?>" style="position:fixed;left:<?php echo $xzzb;?>;top:<?php echo $yzzb;?>;background:#00DDDD;">
					<?php echo $row["测点编号"];?>
				</div>
				<?php }?>	
		  ...
		  <input class="hidden" id="sclb" value="<?php echo $sclb;?>"/>
			...
			<div style="position:fixed;left:350px;top:0px">
				<div class="row col-md-12">
					...
					<form method="post" class="form-horizontal col-md-2">	
						<input type="text" id="sxlba1" name="sxlba" class="hidden" value="普遍开裂" />
						<button type="submit"  class="btn btn-success">普遍开裂</button>	
					</form>		
					<form method="post" class="form-horizontal col-md-2">
						<input type="text" id="sxlba2" name="sxlba" class="hidden" value="有吊顶天花" />	
						<button type="submit"  class="btn btn-primary">有吊顶天花</button>	
					</form>	
				 <form method="post" class="form-horizontal col-md-2">	
						<input type="text" id="sxlba3" name="sxlba" class="hidden" value="无吊顶天花" />
						<button type="submit"  class="btn btn-warning">无吊顶天花</button>	
					</form>		
					<form method="post" class="form-horizontal col-md-2">	
						<input type="text" id="sxlba4" name="sxlba" class="hidden" value="线条" />
						<button type="submit"  class="btn btn-info">线条</button>	
					</form>		
...
////					   echo $sxlba;
					   ...
					   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='天花工程' ";
						 	...
						 	$sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='天花工程' and 测点类别  LIKE '%$sxlba%' ";
						 }
					   $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	        ?>
