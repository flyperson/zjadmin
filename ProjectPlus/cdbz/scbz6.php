			...
			<div style="position:fixed;left:350px;top:0px">
				<div class="row col-md-12">
					...
					<form method="post" class="form-horizontal col-md-3">	
						<input type="text" id="sxlba1" name="sxlba" class="hidden" value="找平层平整度" />
						<button type="submit"  class="btn btn-warning">找平层平整度</button>	
					</form>		
					<form method="post" class="form-horizontal col-md-2">
						<input type="text" id="sxlba2" name="sxlba" class="hidden" value="木地板平整度" />	
						<button type="submit"  class="btn btn-primary">木地板平整度</button>	
					</form>	
...
////					   echo $sxlba;
					   ...
					   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='木地板安装' ";
						 	...
						 	$sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='木地板安装' and 测点类别  LIKE '%$sxlba%' ";
						 }
					   $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	        ?>
