		  ...
		  <input class="hidden" id="sclb" value="<?php echo $sclb;?>"/>
			...
			<div style="position:fixed;left:350px;top:0px">
				<div class="row col-md-12">
					...
					<form method="post" class="form-horizontal col-md-2">	
						<input type="text" id="sxlba1" name="sxlba" class="hidden" value="外墙铺贴" />
						<button type="submit"  class="btn btn-warning">外墙铺贴</button>	
					</form>		
					<form method="post" class="form-horizontal col-md-2">
						<input type="text" id="sxlba2" name="sxlba" class="hidden" value="外墙涂料" />	
						<button type="submit"  class="btn btn-primary">外墙涂料</button>	
					</form>	
						<form method="post" class="form-horizontal col-md-3">	
						<input type="text" id="sxlba3" name="sxlba" class="hidden" value="外墙石材" />	
						<button type="submit"  class="btn btn-info">外墙石材</button>
					</form>	
...
////					   echo $sxlba;
					   ...
					   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='外墙工程' ";
						 	...
						 	$sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='外墙工程' and 测点类别  LIKE '%$sxlba%' ";
						 }
					   $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	        ?>
