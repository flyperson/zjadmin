		  ...
		  <input class="hidden" id="sclb" value="<?php echo $sclb;?>"/>
			...
			<div style="position:fixed;left:350px;top:0px">
				<div class="row col-md-12">
					...
					<form method="post" class="form-horizontal col-md-2">	
						<input type="text" id="sxlba1" name="sxlba" class="hidden" value="天花" />
						<button type="submit"  class="btn btn-warning">天花</button>	
					</form>		
					<form method="post" class="form-horizontal col-md-2">
						<input type="text" id="sxlba2" name="sxlba" class="hidden" value="墙面" />	
						<button type="submit"  class="btn btn-primary">墙面</button>	
					</form>	
					<form method="post" class="form-horizontal col-md-2">
						<input type="text" id="sxlba3" name="sxlba" class="hidden" value="地面" />	
						<button type="submit"  class="btn btn-danger">地面</button>	
					</form>	
...
////					   echo $sxlba;
					   ...
					   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='电梯前室、首层大堂' ";
						 	...
						 	$sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='电梯前室、首层大堂' and 测点类别  LIKE '%$sxlba%' ";
						 }
					   $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	        ?>
