...
//				echo $sclb;
			...
			if($cdlbaa=='墙面砖垂直度(瓷砖墙面)' ||$cdlbaa=='墙面砖垂直度(石材墙面)'||$cdlbaa=='墙面砖垂直度(木质墙面)'){
			?>
			...
			?>
				...
				<?php }?>	
					<?php
						if($cdlbaa=="地面砖表面平整度(地砖地面)"||$cdlbaa=="地面砖表面平整度(石材地面)"){
							?>
			...
			<div style="position:fixed;left:350px;top:0px">
				<div class="row col-md-12">
					...
					<form method="post" class="form-horizontal col-md-3">	
						<input type="text" id="sxlba1" name="sxlba" class="hidden" value="墙面砖垂直度" />
						<button type="submit"  class="btn btn-warning">墙面砖垂直度</button>	
					</form>		
					<form method="post" class="form-horizontal col-md-3">
						<input type="text" id="sxlba2" name="sxlba" class="hidden" value="墙面砖接缝高低差" />	
						<button type="submit"  class="btn btn-primary">墙面砖接缝高低差</button>	
					</form>	
					<form method="post" class="form-horizontal col-md-3">	
						<input type="text" id="sxlba3" name="sxlba" class="hidden" value="地面砖表面平整度" />	
						<button type="submit"  class="btn btn-info">地面砖表面平整度</button>
					</form>	
					<form method="post" class="form-horizontal col-md-3">	
						<input type="text" id="sxlba4" name="sxlba" class="hidden" value="地面砖接缝高低差" />	
						<button type="submit"  class="btn btn-danger">地面砖接缝高低差</button>
					</form>	
...
////					   echo $sxlba;
					   ...
					   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='电梯前室、首层大堂' ";
						 	...
						 	$sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='电梯前室、首层大堂' and 测点类别  LIKE '%$sxlba%' ";
						 }
					   $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	        ?>
