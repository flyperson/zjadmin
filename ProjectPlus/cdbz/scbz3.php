...
//				echo $sclb;
			...
			?>
				<div class="win btn-xs btn-default" onclick="cdaniu(this.id);" name="<?php echo $row["测点编号"];?>&<?php echo $id;?>" id="<?php echo $row["测点编号"];?>&<?php echo $id;?>"  data-toggle="tooltip" data-placement="right" title="<?php echo $row["测点类别"];?>,<?php echo $row["测点实测值"];?>" style="position:fixed;left:<?php echo $xzzb;?>;top:<?php echo $yzzb;?>;">
					<?php echo $row["测点编号"];?>
				</div>
			...
			<div style="position:fixed;left:350px;top:0px">
				<div class="row col-md-12">
					...
					<form method="post" class="form-horizontal col-md-4">	
						<input type="text" id="sxlba1" name="sxlba" class="hidden" value="座便坑距偏差" />
						<button type="submit"  class="btn btn-warning">座便坑距偏差</button>	
					</form>		
					<!--<form method="post" class="form-horizontal col-md-2">
						<input type="text" id="sxlba2" name="sxlba" class="hidden" value="垂直度" />	
						<button type="submit"  class="btn btn-primary">垂直度</button>	
					</form>	
					<form method="post" class="form-horizontal col-md-3">	
						<input type="text" id="sxlba3" name="sxlba" class="hidden" value="外窗洞口尺寸" />	
						<button type="submit"  class="btn btn-info">外窗洞口尺寸</button>
					</form>	-->
					<!--<form method="post" class="form-horizontal col-md-2">	
						<input type="text" id="sxlba4" name="sxlba" class="hidden" value="顶板水平度" />	
						<button type="submit"  class="btn btn-danger">顶板水平度</button>
					</form>	
					<form method="post" class="form-horizontal col-md-2">		
						<input type="text" id="sxlba5" name="sxlba" class="hidden" value="楼板厚度" />
						<button type="submit"  class="btn btn-success">楼板厚度</button>
					</form>		-->
				</div>
...
////					   echo $sxlba;
					   ...
					   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='设备安装工程' ";
						 	...
						 	$sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='设备安装工程' and 测点类别  LIKE '%$sxlba%' ";
						 }
					   $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	        ?>
