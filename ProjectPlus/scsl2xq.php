...
//										  $gcid1=$_GET["gcid"];
										  $yhid=$_GET["yhid"];
											...
											$sql = "select * from 建筑工程  where 项目类别='抹灰工程 'and id='$id' ";
                     ...
                     ?>
						...
						<button type="button" class="btn btn-default " data-dismiss="modal">		<a href="scsl2.php?id=<?php echo $yhid=$_GET["gcid"];?>&yhid=<?php echo $yhid=$_GET["yhid"]; ?>">返回上一层
						</a></button>
		...
		<div role="tabpanel" class="tab-pane fade " id="scslsj">
			...
			<form id="cdbcform" name="cdbcform"  method="post" class="form-horizontal" role="form" >
                   ...
                   require("../conn.php");   
				   				 $jcid=$_GET["id"];                
                   $sql = "select * from  新建测点 where 项目类别='抹灰工程' and 建筑工程id='$jcid' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
            <div class="form-group">
							...
							<div class="form-group">
								...
								<?php
		   						...
		   						?>
									...
									<input type="text" class="form-control hidden" id="gclb" name="gclb" value="抹灰工程">						
