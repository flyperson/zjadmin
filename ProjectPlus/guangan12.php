    				...
    				<ul class="nav bs-docs-sidenav"> 					
    					...
    					<li>
    						...
    						<ul class="nav">
    							...
    							<li class="active"><a href="guangan12.php?gcid=<?php echo $row["id"];?>&yhid=<?php echo $row["用户id"];?>&zljcid=<?php echo $_GET["zljcid"];?>&sjc=<?php echo $_GET["sjc"];?>">电梯前室、首层大堂 </a></li></br>
    						</ul>
    					</li>   
...
//										  $gcid1=$_GET["gcid"];
									 ...
									 ?>
										<div class="form-group">						
											...
											<?php
											  require("../conn.php");
											  $zljcid=$_GET["zljcid"];
												$sql = "select * from 测点布置附件表  where 质量检查id='$zljcid' and 实测类别='观感评价' and 项目类别='电梯前室、首层大堂'  ";
												$result = $conn->query($sql);
												while($row = $result->fetch_assoc()) {
                      ?>
        ...
        <button type="button" class="btn btn-default"  onclick="javascript:window.open('cdbz/ggbz12.php?id=<?php echo $_GET["zljcid"];?>&xmlb=<?php echo '电梯前室、首层大堂'?>&sclb=<?php echo '观感评价'?>', 'newwindow2', 'height=420, width=315, top=0, left=500,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')">
        	测点布置
        </button>
			</div>
				<p></p>
		...
		<a href="#" id="myTabDrop1" class="dropdown-toggle" 
		   data-toggle="dropdown">测点类别筛选 <b class="caret"></b>
									...
									<ul class="dropdown-menu" role="menu" aria-labelledby="myTabDrop1" >
										...
										<li class=""><a href="#cd1"  tabindex="-1" data-toggle="tab">天花 </a></li>
										<li class=""><a href="#cd2"  tabindex="-1" data-toggle="tab">墙面</a></li>
										<li class=""><a href="#cd3"  tabindex="-1" data-toggle="tab">地面 </a></li>
									</ul>	
									<li>
									</ul>
			...
			<form id="cdbcform" name="cdbcform"  method="post" class="form-horizontal" role="form" >    							
                   ...
                   require("../conn.php");  
				   				 ...
				   				 $zljcid=$_GET["zljcid"]; 
								   	 ...
								   	 $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='电梯前室、首层大堂' ";
             ...
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='电梯前室、首层大堂' and 测点类别='天花' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='电梯前室、首层大堂' and 测点类别='墙面' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='电梯前室、首层大堂' and 测点类别='地面' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
              <div class="form-group" id="ceshiyong">
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='电梯前室、首层大堂' and 测点类别 = '天花' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='电梯前室、首层大堂' and 测点类别 = '墙面' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='电梯前室、首层大堂' and 测点类别 = '地面' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
					...
					<div class="form-group col-sm-12" >
						...
						<div class="col-sm-2">
							<input type="text" class="form-control" id="cdlbA" name="cdlbA" value="天花" >
						</div>
					...
					<div class="form-group col-sm-12" >
						...
						<div class="col-sm-2">
							<input type="text" class="form-control" id="qbmpzd" name="qbmpzd" value="墙面" >
						</div>
					...
					<div class="form-group col-sm-12" >
						<label for="cdlbA" class="col-sm-2 control-label">测点类别：</label>
						<div class="col-sm-2">
							<input type="text" class="form-control" id="cdlbA" name="cdlbA" value="地面" >
						</div>
						<label for="hgl" class="col-sm-2 control-label">合格率：</label>
						<div class="col-sm-2">
			...
			<div class="modal fade" id="myModal1" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
					...
					<div class="form-group">
						...
						<label for="cdlx1" class="col-sm-3 control-label">测点类型：</label>
										<div class="col-sm-3">
											<select id="cdlx1" name="cdlx1" class="form-control">
							                   ...
							                   require("../conn.php");  
							   				  $zljcid=$_GET["zljcid"]; 
											   $sql = "select 测点类型 from  观感评价测点类型表 where  工程类别='电梯前室、首层大堂'and 测点类别='天花'  ";
							                   $result = $conn->query($sql);
								                   while($row = $result->fetch_assoc()) {
								                   ?>
											...
											<select id="cdlx2" name="cdlx2" class="form-control hidden">
							                   ...
							                   require("../conn.php");  
							   				  $zljcid=$_GET["zljcid"]; 
											   $sql = "select 测点类型 from  观感评价测点类型表 where  工程类别='电梯前室、首层大堂'and 测点类别='墙面'  ";
							                   $result = $conn->query($sql);
								                   while($row = $result->fetch_assoc()) {
								                   ?>
												<option><?php echo $row["测点类型"];?></option>
											...
											<select id="cdlx3" name="cdlx3" class="form-control hidden">
							                   ...
							                   require("../conn.php");  
							   				  $zljcid=$_GET["zljcid"]; 
											   $sql = "select 测点类型 from  观感评价测点类型表 where  工程类别='电梯前室、首层大堂'and 测点类别='地面'  ";
							                   $result = $conn->query($sql);
								                   while($row = $result->fetch_assoc()) {
								                   ?>
												<option><?php echo $row["测点类型"];?></option>
						...
						<div class="form-group ">	
								...
								<div class="col-sm-9">
									<textarea class="form-control" rows="6" id="cq"  name="cq">抽检10个电梯前室、首层大堂作为10个测区（不足10个以实计算）</textarea>	
								</div>	
							</div>
									<div class="form-group ">
	      ...
	      $('#cdlx1').addClass("hidden");
		    $('#cdlx2').addClass("hidden");	
		     $('#cdlx3').addClass("hidden");
				...
				else if (cdlb.value=='墙面')
				{ 				
					$('#cdlx2').removeClass("hidden");			
				}
				else if (cdlb.value=='地面')
				{ 				
				...
				$("#cdlx1").click(function(){ 
			 	...
			 	if(cdlx1=="工艺"){
			 		document.getElementById("nr").value='吊杆、龙骨、面板安装、固定符合要求，防锈、防火、防腐等符合要求，线条部件安装符合要求'
			 		
			 	}
			 	else if	(cdlx1=="观感"){
			 		document.getElementById("nr").value='天花平整，阴阳角顺直，无开裂、返锈、色差、脱皮等现象，各部件、接口收口美观'
			 ...
			 $("#cdlx2").click(function(){ 
			 	...
			 	var cdlx2=$("#cdlx2").val();
			 	if(cdlx2=="工艺"){
			 		$("#nr").val('基层处理到位，防潮、防腐部位按要求施工，石材、瓷砖按要求铺贴或挂设')
			 		
			 	}
			 	else if	(cdlx2=="观感"){
			 		$("#nr").val('涂饰平整、洁净，无裂缝、潮湿、脱皮等；瓷砖、石材按要求排版，缝隙均匀，无色差，边角收口美观，无崩损、脱落、空鼓等；各部件、接口收口美观')
			...
			$("#cdlx3").click(function(){
				...
				if(cdlx3=="工艺"){
					...
					$("#nr").val('排版合理，缝隙均匀，回缝密实、饱满，无色差、崩损、空鼓、大小头等')
				}
