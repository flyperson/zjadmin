    				...
    				<ul class="nav bs-docs-sidenav"> 					
    					...
    					<li><b style="margin-left:20px;">观感</b>
    						...
    						<ul class="nav">
    							...
    							<li class="active"><a href="guangan5.php?gcid=<?php echo $row["id"];?>&yhid=<?php echo $row["用户id"];?>&zljcid=<?php echo $_GET["zljcid"];?>&sjc=<?php echo $_GET["sjc"];?>">门窗工程</a></li>
...
//										  $gcid1=$_GET["gcid"];
										...
										<!--<h3 class="panel-title label label-info"></h3>--><HR style="FILTER: progid:DXImageTransform.Microsoft.Shadow(color:#987cb9,direction:145,strength:15)" width="100%" color=#987cb9 SIZE=1>
												...
												<div class="form-group">
													...
													<div class="col-sm-3">
														...
														<input type="text" class="hidden" id="xmlb" name="xmlb" value="门窗工程">
									 ...
									 ?>
										<div class="form-group">						
											...
											<?php
											  require("../conn.php");
											  $zljcid=$_GET["zljcid"];
												$sql = "select * from 测点布置附件表  where 质量检查id='$zljcid' and 实测类别='观感评价' and 项目类别='门窗工程'  ";
												$result = $conn->query($sql);
												while($row = $result->fetch_assoc()) {
                      ?>
        ...
        <button type="button" class="btn btn-default"  onclick="javascript:window.open('cdbz/ggbz5.php?id=<?php echo $_GET["zljcid"];?>&xmlb=<?php echo '门窗工程'?>&sclb=<?php echo '观感评价'?>', 'newwindow2', 'height=470, width=1100, top=100, left=200,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')">
        	测点布置
        </button>
			</div>
				<p></p>
		...
		<a href="#" id="myTabDrop1" class="dropdown-toggle" 
		   data-toggle="dropdown">测点类别筛选 <b class="caret"></b>
             ...
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='门窗工程' and 测点类别='工艺要求' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='门窗工程' and 测点类别='观感质量' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
              <div class="form-group" id="ceshiyong">
			...
			<div role="tabpanel" class="tab-pane fade " id="tjfx">
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='门窗工程' and 测点类别 = '工艺要求' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='门窗工程' and 测点类别 = '观感质量' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
...
（不足10个以实计算）</textarea>	
							...
							<div class="col-sm-3">
											<input type="text" class="form-control hidden" id="xmlb" name="xmlb" value="门窗工程" placeholder="">
		 ...
		 $("#cdbc").click(function(){ 
			...
			if (cdlb.value=='工艺要求')
			{
			 ...
			 $("#cdlx1").click(function(){ 
			 	...
			 	if(cdlx1=="洞口尺寸"){
			 		document.getElementById("nr").value='门窗框与洞口墙体安装缝隙过宽或过窄的（小于10mm（每边）或大于30mm（每边）），需提前按要求进行处理，大于30mm的需在窗框塞缝完成后再涂刷防水涂料至框边10cm';
			 		$("#cdsl").val("10");
			 		
			 	}
			 	else if	(cdlx1=="固定"){
			 		document.getElementById("nr").value='门窗框固定码按要求进行设置并进行有效固定；落地门窗下框未座浆固定';
			 		$("#cdsl").val("10");
			 		...
			 		document.getElementById("nr").value='门窗外框塞缝及安装须符合要求；严禁缠绕门窗框保护膜未拆除就填塞';
			 		$("#cdsl").val("10");
			 		
			 	}
			 	
			 	 else if (cdlx1="成品保护"){
			 	 	document.getElementById("nr").value='门窗框要求采取有效的成品保护措施';
			 	 	$("#cdsl").val("10");
			 	 }
			 ...
			 $("#cdlx2").click(function(){ 
			 	...
			 	if(cdlx2=="渗漏"){
			 		...
			 		$("#nr").val('窗边抹灰无空鼓开裂；外窗台按设计要求放坡');
			 		$("#cdsl").val("10");
			 		
			 	}
			 	else if	(cdlx2=="打胶"){
			 		$("#nr").val('窗边打胶前基层处理到位，打胶观感符合要求；无固定码外露');
			 		$("#cdsl").val("10");
