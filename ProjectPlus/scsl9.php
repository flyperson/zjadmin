    				...
    				<ul class="nav bs-docs-sidenav"> 					
    					...
    					<li>
    						...
    						<ul class="nav">
    							...
    							<li class="active"><a href="scsl9.php?gcid=<?php echo $row["id"];?>&yhid=<?php echo $row["用户id"];?>&zljcid=<?php echo $_GET["zljcid"];?>&sjc=<?php echo $_GET["sjc"];?>">地面饰面砖工程</a></li>
...
//										  $gcid1=$_GET["gcid"];
										...
										<!--<h3 class="panel-title label label-info"></h3>--><HR style="FILTER: progid:DXImageTransform.Microsoft.Shadow(color:#987cb9,direction:145,strength:15)" width="100%" color=#987cb9 SIZE=1>
												...
												<div class="form-group">
													...
													<div class="col-sm-3">
														...
														<input type="text" class="hidden" id="xmlb" name="xmlb" value="地面饰面砖">
									 ...
									 ?>
										<div class="form-group">						
											...
											<?php
											  require("../conn.php");
											  $zljcid=$_GET["zljcid"];
												$sql = "select * from 测点布置附件表  where 质量检查id='$zljcid' and 实测类别='实测实量' and 项目类别='地面饰面砖' ";
												$result = $conn->query($sql);
												while($row = $result->fetch_assoc()) {
                      ?>
        ...
        <button type="button" class="btn btn-default"  onclick="javascript:window.open('cdbz/scbz9.php?id=<?php echo $_GET["zljcid"];?>&xmlb=<?php echo '地面饰面砖' ?>&sclb=<?php echo '实测实量' ?>', 'newwindow2', 'height=470, width=1100, top=100, left=200,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')">
        	测点布置                                                                                                                                                                                                                                   
        </button>
		...
		<a href="#" id="myTabDrop1" class="dropdown-toggle" 
		   data-toggle="dropdown">测点类别筛选 <b class="caret"></b>
									...
									<ul class="dropdown-menu" role="menu" aria-labelledby="myTabDrop1" >
										...
										<li class=""><a href="#cd2"  tabindex="-1" data-toggle="tab">接缝高低差</a></li>
			...
			<form id="cdbcform" name="cdbcform"  method="post" class="form-horizontal" role="form" >    							
                   ...
                   require("../conn.php");  
				   				 ...
				   				 $zljcid=$_GET["zljcid"]; 
								   	...
								   	$sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='地面饰面砖' ";
            ...
            <div class="form-group" id="ceshiyong">
                   ...
                   require("../conn.php");  
				   				 error_reporting(E_ALL ^ E_NOTICE);	 
				   				 $zljcid=$_GET["zljcid"]; 
								   $sql1 = "select * from  装修工程质量检查表  where  检查项目='地面饰面砖' and 检查内容='$xmlB' ";
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='地面饰面砖' and 测点类别 LIKE '表面平整度%' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   $xmlB = $row["测点类别"];
                   ?>
            <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='地面饰面砖' and 测点类别='接缝高低差' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   $xmlB = $row["测点类别"];
                   ?>
...
//							   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='混凝土结构工程' ";
             $sql = "select 测点实测值 from 实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='地面饰面砖' and 测点类别 = '表面平整度' order by CONVERT(测点实测值,SIGNED) desc ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
//							   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='混凝土结构工程' ";
             $sql = "select 测点实测值 from 实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='地面饰面砖' and 测点类别 = '接缝高低差' order by CONVERT(测点实测值,SIGNED) desc ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
					...
					<div class="form-group col-sm-12" >
						...
						<div class="col-sm-2">
							<input type="text" class="form-control" id="qbmpzd" name="qbmpzd" value="接缝高低差" >
						</div>
					...
					<div class="form-group">
						<label for="cdlb" class="col-sm-3 control-label">测点类别：</label>
								<div class="col-sm-3">
				                   ...
				                   require("../conn.php");   
								   				 $zljcid=$_GET["zljcid"];                
				                   $sql = "select distinct 检查内容 from  装修工程质量检查表  where 检查项目='地面饰面砖'";
				                   $result = $conn->query($sql);
				                   while($row = $result->fetch_assoc()) {
				                ?>
						...
						<div class="form-group ">
							...
							<div class="col-sm-3">
								<input type="text" class="form-control hidden" id="xmlb" name="xmlb" value="地面饰面砖" placeholder="">
...
<script type="text/javascript">
 $("#cdlb").click(function(){ 
			 	...
			 	if(cdlb=="表面平整度(地砖地面)"){
			 		document.getElementById("nr").value='同一户选取2个功能房（1）每一功能房间饰面砖地面都可以作为1个实测区，累计实测实量10 个实测区。如该户房内还有石材地面，则增加10个实测区；（2）每一功能房间地面（不包括厨卫间）的4个角部区域，任选两个角与墙面夹角45度平放靠尺共测量2次。客餐厅或较大房间地面的中部区域需加测1次。1个实测值作为判断该实测指标合格率的1个计算点。';
			 		$("#cdsl").val("20");
			 		...
			 		document.getElementById("nr").value='同一户选取10个检测点（1）每户作为一个实测区，累计实测实量5 个测区；（2）每个测区选择10个检测点，原则上客厅不少于5个，餐厅不少于2个，阳台不少于1个，饰面砖踢脚线不少于2个。（3）用钢尺或其他辅助工具紧靠相邻两饰面砖跨过接缝，用0.5MM钢塞片插入钢尺与饰面砖之间的缝隙。如能插入，则该测量点不合格；反之则该测量点合格。每条接缝高低差的测量值，均作为判断该实测指标合格率的1个计算点。（4） 为数据统计方便和提高实测效率，不合格点均按0.7mm 记录，合格点均按0.3mm记录。';
			 		$("#cdsl").val("50");
