    				...
    				<ul class="nav bs-docs-sidenav"> 					
    					...
    					<li>
    						...
    						<ul class="nav">
    							...
    							<li class="active"><a href="guangan15.php?gcid=<?php echo $row["id"];?>&yhid=<?php echo $row["用户id"];?>&zljcid=<?php echo $_GET["zljcid"];?>&sjc=<?php echo $_GET["sjc"];?>">厨房</a></li>
...
//										  $gcid1=$_GET["gcid"];
										...
										<!--<h3 class="panel-title label label-info"></h3>--><HR style="FILTER: progid:DXImageTransform.Microsoft.Shadow(color:#987cb9,direction:145,strength:15)" width="100%" color=#987cb9 SIZE=1>
												...
												<div class="form-group">
													...
													<div class="col-sm-3">
														...
														<input type="text" class="hidden" id="xmlb" name="xmlb" value="厨房">
									 ...
									 ?>
										<div class="form-group">						
											...
											<?php
											  require("../conn.php");
											  $zljcid=$_GET["zljcid"];
												$sql = "select * from 测点布置附件表  where 质量检查id='$zljcid' and 实测类别='观感评价' and 项目类别='厨房'  ";
												$result = $conn->query($sql);
												while($row = $result->fetch_assoc()) {
                      ?>
        ...
        <button type="button" class="btn btn-default"  onclick="javascript:window.open('cdbz/ggbz15.php?id=<?php echo $_GET["zljcid"];?>&xmlb=<?php echo '厨房'?>&sclb=<?php echo '观感评价'?>', 'newwindow2', 'height=420, width=315, top=0, left=500,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')">
        	测点布置
        </button>
			</div>
				<p></p>
		...
		<a href="#" id="myTabDrop1" class="dropdown-toggle" 
		   data-toggle="dropdown">测点类别筛选 <b class="caret"></b>
									...
									<ul class="dropdown-menu" role="menu" aria-labelledby="myTabDrop1" >
										...
										<li class=""><a href="#cd1"  tabindex="-1" data-toggle="tab">橱柜 </a></li>
										<li class=""><a href="#cd2"  tabindex="-1" data-toggle="tab">电器</a></li>
										<li class=""><a href="#cd3"  tabindex="-1" data-toggle="tab">给、排水</a></li>
									</ul>	
									<li>
									</ul>
			...
			<form id="cdbcform" name="cdbcform"  method="post" class="form-horizontal" role="form" >    							
                   ...
                   require("../conn.php");  
				   				 ...
				   				 $zljcid=$_GET["zljcid"]; 
								   	 ...
								   	 $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='厨房' ";
             ...
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='厨房' and 测点类别='橱柜' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='厨房' and 测点类别='电器' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='厨房' and 测点类别='给、排水' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
              <div class="form-group" id="ceshiyong">
			...
			<div role="tabpanel" class="tab-pane fade " id="tjfx">
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='厨房' and 测点类别 = '橱柜' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='厨房' and 测点类别 = '电器' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='厨房' and 测点类别 = '给、排水' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
					...
					<div class="form-group col-sm-12" >
						...
						<div class="col-sm-2">
							<input type="text" class="form-control" id="cdlbA" name="cdlbA" value="橱柜" >
						</div>
					...
					<div class="form-group col-sm-12" >
						...
						<div class="col-sm-2">
							<input type="text" class="form-control" id="qbmpzd" name="qbmpzd" value="电器" >
						</div>
			...
			<div class="modal fade" id="myModal1" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
					...
					<div class="form-group">
						...
						<label for="cdlx1" class="col-sm-3 control-label">测点类型：</label>
										<div class="col-sm-3">
											<select id="cdlx1" name="cdlx1" class="form-control">
							                   ...
							                   require("../conn.php");  
							   				  $zljcid=$_GET["zljcid"]; 
											   $sql = "select 测点类型 from  观感评价测点类型表 where  工程类别='厨房'and 测点类别='橱柜'  ";
							                   $result = $conn->query($sql);
								                   while($row = $result->fetch_assoc()) {
								                   ?>
											...
											<select id="cdlx2" name="cdlx2" class="form-control hidden">
							                   ...
							                   require("../conn.php");  
							   				  $zljcid=$_GET["zljcid"]; 
											   $sql = "select 测点类型 from  观感评价测点类型表 where  工程类别='厨房'and 测点类别='电器'  ";
							                   $result = $conn->query($sql);
								                   while($row = $result->fetch_assoc()) {
								                   ?>
												<option><?php echo $row["测点类型"];?></option>
											...
											<select id="cdlx3" name="cdlx3" class="form-control hidden">
							                   ...
							                   require("../conn.php");  
							   				  $zljcid=$_GET["zljcid"]; 
											   $sql = "select 测点类型 from  观感评价测点类型表 where  工程类别='厨房'and 测点类别='给、排水'  ";
							                   $result = $conn->query($sql);
								                   while($row = $result->fetch_assoc()) {
								                   ?>
												<option><?php echo $row["测点类型"];?></option>
						...
						<div class="form-group ">	
							...
							<div class="col-sm-3">
											<input type="text" class="form-control hidden" id="xmlb" name="xmlb" value="厨房" placeholder="">
		    ...
		    $('#cdlx2').addClass("hidden");	
		     $('#cdlx3').addClass("hidden");
				...
				$("#cdlx1").click(function(){ 
			 	...
			 	if(cdlx1=="背板"){
			 		document.getElementById("nr").value='做防潮、防腐处理 '
			 		
			 	}
			 	else if	(cdlx1=="安装"){
			 		document.getElementById("nr").value='用玻璃钻对瓷砖开孔'
			 		
			 	}
			 	
			 	 	else if	(cdlx1=="表面"){
			 		document.getElementById("nr").value='柜体安装满足设计要求，牢固，水平、垂直，无破损、划伤、污染、色差'
			 		
			 	}
			 	else if	(cdlx1=="间距"){
			 		document.getElementById("nr").value='台面与吊柜间距符合规范要求，相邻柜身接触面缝隙符合要求，门扇与门扇间缝隙均匀、水平'
			 		
			 	}
			 	
			 	 	else if	(cdlx1=="细部"){
			 		document.getElementById("nr").value='橱柜五金件齐全、安装牢固，开启灵活，使用功能正常，无松脱、晃动；洗手盆下水出口底板孔洞完整（不大于∅10cm),冷热水接驳口背板开口不大于∅5cm'
			 ...
			 $("#cdlx2").click(function(){ 
			 	...
			 	if(cdlx2=="安装"){
			 		...
			 		$("#nr").val('与橱柜缝隙必须均匀（小于5mm)，与橱柜面同一立面')
			 		...
			 		$("#nr").val('安装牢固，与吊柜的缝隙小于1cm')
			 		
			 	}
			 	else if(cdlx2=="嵌入式冰箱"){
			 		$("#nr").val('与橱柜间的缝隙均匀、合理，非嵌入式冰箱与预留位匹配，所有电器均无损坏、刮痕')
			 	}
			});
			$("#cdlx3").click(function(){
				var cdlx3=$("#cdlx3").val();
				if(cdlx3=="给、排水管"){
					$("#nr").val('牢固、位置正确、坡度正确，接口无渗漏 ')
				}
				else if(cdlx3=="龙头"){
					$("#nr").val('安装牢固，无污渍、渗漏；无刮花、破损，角阀与墙面接口平整美观')
				}
				else if(cdlx3=="洗菜盆"){
					$("#nr").val('安装牢固、与台面封胶均匀，去水安装完整')
				}
