...
//										echo $_GET["id"];
    				...
    				<ul class="nav bs-docs-sidenav"> 					
    					...
    					<li><b style="margin-left:20px;">观感</b>
    						...
    						<ul class="nav">
    							...
    							<li class="active"><a href="guangan7.php?gcid=<?php echo $row["id"];?>&yhid=<?php echo $row["用户id"];?>&zljcid=<?php echo $_GET["zljcid"];?>&sjc=<?php echo $_GET["sjc"];?>">水电安装工程</a></li>
	...
	<div class="panel-heading">
		<h3 class="panel-title">水电安装工程</h3>
	</div>
...
//										  $gcid1=$_GET["gcid"];
										...
										<!--<h3 class="panel-title label label-info"></h3>--><HR style="FILTER: progid:DXImageTransform.Microsoft.Shadow(color:#987cb9,direction:145,strength:15)" width="100%" color=#987cb9 SIZE=1>
												...
												<div class="form-group">
													...
													<div class="col-sm-3">
														...
														<input type="text" class="hidden" id="xmlb" name="xmlb" value="水电安装工程">
									 ...
									 ?>
										<div class="form-group">						
											...
											<?php
											  require("../conn.php");
											  $zljcid=$_GET["zljcid"];
												$sql = "select * from 测点布置附件表  where 质量检查id='$zljcid' and 实测类别='观感评价' and 项目类别='水电安装工程'  ";
												$result = $conn->query($sql);
												while($row = $result->fetch_assoc()) {
                      ?>
        ...
        <button type="button" class="btn btn-default"  onclick="javascript:window.open('cdbz/ggbz7.php?id=<?php echo $_GET["zljcid"];?>&xmlb=<?php echo '水电安装工程'?>&sclb=<?php echo '观感评价'?>', 'newwindow2', 'height=470, width=1100, top=100, left=200,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')">
        	测点布置
        </button>
			</div>
				<p></p>
		...
		<a href="#" id="myTabDrop1" class="dropdown-toggle" 
		   data-toggle="dropdown">测点类别筛选 <b class="caret"></b>
			...
			<form id="cdbcform" name="cdbcform"  method="post" class="form-horizontal" role="form" >    							
                   ...
                   require("../conn.php");  
				   				 ...
				   				 $zljcid=$_GET["zljcid"]; 
								   	 ...
								   	 $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='水电安装工程' ";
              ...
              <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='水电安装工程' and 测点类别='工艺要求' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='水电安装工程' and 测点类别='观感质量' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
              <div class="form-group" id="ceshiyong">
			...
			<div role="tabpanel" class="tab-pane fade " id="tjfx">
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='水电安装工程' and 测点类别 = '工艺要求' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='水电安装工程' and 测点类别 = '观感质量' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
...
（不足10个以实计算）</textarea>	
							...
							<div class="col-sm-3">
											<input type="text" class="form-control hidden" id="xmlb" name="xmlb" value="水电安装工程" placeholder="">
        ...
        $("#save3").click(function(){ 
		 ...
		 $("#cdbc").click(function(){ 
			...
			$("#cdlx1").click(function(){ 
			 	...
			 	if(cdlx1=="套管"){
			 		document.getElementById("nr").value='出屋面、阳露台、卫生间防水套管按要求预埋';
			 		$("#cdsl").val("10");
			 		
			 	}
			 	else if	(cdlx1=="给排水"){
			 		document.getElementById("nr").value='沉箱内及阳台给、排水管道位置、标高及坡向正确、按要求铺设安装、固定';
			 		$("#cdsl").val("10");
			 		
			 	}
			 	
			 	 	else if	(cdlx1=="布管布线"){
			 		document.getElementById("nr").value='暗敷管道按要求开槽、不允许破坏防水层贴墙安装；开槽禁止未使用机械切割而直接人工打凿；墙面布管按要求填塞或挂网，线管、电箱、线盒安装符合要求；电气底盒未突出抹灰完成面；裸露线头按要求保护';
			 		$("#cdsl").val("10");
			 		
			 	}
			 	
			 	 else if (cdlx1="空调洞"){
			 	 	document.getElementById("nr").value='空调孔洞位置、坡度正确，砼结构内套管按要求预留';
			 	 	$("#cdsl").val("10");
			 	 }
