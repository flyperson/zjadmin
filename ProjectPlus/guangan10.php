    				...
    				<ul class="nav bs-docs-sidenav"> 					
    					...
    					<li>
    						...
    						<ul class="nav">
    							...
    							<li class="active"><a href="guangan10.php?gcid=<?php echo $row["id"];?>&yhid=<?php echo $row["用户id"];?>&zljcid=<?php echo $_GET["zljcid"];?>&sjc=<?php echo $_GET["sjc"];?>">天花工程</a></li>
...
//										  $gcid1=$_GET["gcid"];
										...
										<!--<h3 class="panel-title label label-info"></h3>--><HR style="FILTER: progid:DXImageTransform.Microsoft.Shadow(color:#987cb9,direction:145,strength:15)" width="100%" color=#987cb9 SIZE=1>
												...
												<div class="form-group">
													...
													<div class="col-sm-3">
														...
														<input type="text" class="hidden" id="xmlb" name="xmlb" value="天花工程">
									 ...
									 ?>
										<div class="form-group">						
											...
											<?php
											  require("../conn.php");
											  $zljcid=$_GET["zljcid"];
												$sql = "select * from 测点布置附件表  where 质量检查id='$zljcid' and 实测类别='观感评价' and 项目类别='天花工程'  ";
												$result = $conn->query($sql);
												while($row = $result->fetch_assoc()) {
                      ?>
        ...
        <button type="button" class="btn btn-default"  onclick="javascript:window.open('cdbz/ggbz10.php?id=<?php echo $_GET["zljcid"];?>&xmlb=<?php echo '天花工程'?>&sclb=<?php echo '观感评价'?>', 'newwindow2', 'height=420, width=315, top=0, left=500,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')">
        	测点布置
        </button>
			</div>
				<p></p>
		...
		<a href="#" id="myTabDrop1" class="dropdown-toggle" 
		   data-toggle="dropdown">测点类别筛选 <b class="caret"></b>
									...
									<ul class="dropdown-menu" role="menu" aria-labelledby="myTabDrop1" >
										...
										<li class=""><a href="#cd1"  tabindex="-1" data-toggle="tab">普遍开裂</a></li>
										<li class=""><a href="#cd2"  tabindex="-1" data-toggle="tab">有吊顶天花</a></li>
										<li class=""><a href="#cd3"  tabindex="-1" data-toggle="tab">无吊顶天花</a></li>
										<li class=""><a href="#cd4"  tabindex="-1" data-toggle="tab">线条</a></li>
										</ul>
									<li>
									</ul>
			...
			<form id="cdbcform" name="cdbcform"  method="post" class="form-horizontal" role="form" >    							
                   ...
                   require("../conn.php");  
				   				 ...
				   				 $zljcid=$_GET["zljcid"]; 
								   	 ...
								   	 $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='天花工程' ";
             ...
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='天花工程' and 测点类别='普遍开裂' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='天花工程' and 测点类别='有吊顶天花' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='天花工程' and 测点类别='无吊顶天花' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
              <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='天花工程' and 测点类别='线条' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
             <div class="form-group" id="ceshiyong">
			...
			<div role="tabpanel" class="tab-pane fade " id="tjfx">
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='天花工程' and 测点类别 = '普遍开裂' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='天花工程' and 测点类别 = '有吊顶天花' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='天花工程' and 测点类别 = '无吊顶天花' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='天花工程' and 测点类别 = '线条' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
					   <input type="text" class="form-control hidden" name="xt" value="<?php echo $row["观感评价"];?>" >
					<?php
						}
						$conn->close();					
					?>
					...
					<div class="form-group col-sm-12" >
						...
						<div class="col-sm-2">
							<input type="text" class="form-control" id="cdlbA" name="cdlbA" value="普遍开裂" >
						</div>
					...
					<div class="form-group col-sm-12" >
						...
						<div class="col-sm-2">
							<input type="text" class="form-control" id="qbmpzd" name="qbmpzd" value="有吊顶天花" >
						</div>
					...
					<div class="form-group col-sm-12" >
						<label for="cdlbA" class="col-sm-2 control-label">测点类别：</label>
						<div class="col-sm-2">
							<input type="text" class="form-control" id="cdlbA" name="cdlbA" value="无吊顶天花" >
						</div>
						<label for="hgl" class="col-sm-2 control-label">合格率：</label>
						<div class="col-sm-2">
					...
					<div class="form-group col-sm-12" >
						<label for="qbmpzd" class="col-sm-2 control-label">测点类别：</label>
						<div class="col-sm-2">
							<input type="text" class="form-control" id="qbmpzd" name="qbmpzd" value="线条" >
						</div>
						<label for="hgl1" class="col-sm-2 control-label">合格率：</label>
						<div class="col-sm-2">
			...
			<div class="modal fade" id="myModal1" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
					...
					<div class="form-group">
						...
						<div class="form-group ">	
								...
								<div class="col-sm-9">
									<textarea class="form-control" rows="6" id="cq"  name="cq">抽检10户中的40个天花（客厅、卧室、厨房、卫生间）作为40个测区，需涵盖主力户型
（不足40个以实计算）</textarea>	
								</div>	
							</div>
									<div class="form-group ">
							...
							<div class="col-sm-3">
											<input type="text" class="form-control hidden" id="xmlb" name="xmlb" value="天花工程" placeholder="">
...
//							return callback('ajax错误'+type+errorThrown);
			...
			if (cdlb.value=='有吊顶天花')
			{
			 ...
			 $("#cdlx1").click(function(){ 
			 	...
			 	if(cdlx1=="安装"){
			 		document.getElementById("nr").value='预埋件、连接件、吊杆、龙骨及板材等安装间距、固定方式，防锈、防火、防腐等符合要求'
			 		
			 	}
			 	else if	(cdlx1=="表面"){
			 		document.getElementById("nr").value='牢固、平整，阴阳角顺直、方正，无污染、无开裂，部件收口美观'
			 		
			 	}
			 	
			 	 	else if	(cdlx1=="油漆"){
			 		document.getElementById("nr").value='平整、不粗糙、无色差'
			 ...
			 $("#cdlx2").click(function(){ 
			 	...
			 	if(cdlx2=="基层"){
			 		...
			 		$("#nr").val('天花平整，阴阳角顺直，无开裂')
			 		
			 	}
			 	else if(cdlx2=="油漆"){
			 		$("#nr").val('涂饰均匀一致，表面平整光滑，无返锈、脱皮等现象')
			 	}
			...
			$("#cdlx3").click(function(){
				var cdlx3=$("#cdlx3").val();
				if(cdlx3=="安装"){
					$("#nr").val('基层弹线验收，固定件安装间距符合要求')
				}
				else if(cdlx3=="拼接"){
					$("#nr").val('阴阳角、碰角符合45度拼接要求')
				}
				else if(cdlx3=="表面"){
					$("#nr").val('牢固、顺直，打磨平整、接口顺畅，无裂缝、返锈、变色等现象')
				}
				else if(cdlx3=="油漆"){
					$("#nr").val('颜色均匀一致，表面光滑')
				}
			...
			$("#cdlx4").click(function(){
				var cdlx4=$("#cdlx4").val();
				if(cdlx4=="普遍开裂"){
					$("#nr").val('单户吊顶、线条开裂处数＞3，超出部分每处扣0.2分，每户扣分不超过2分')
				}
			});
</script>
  </body>
...
//    alert(cdsczpl.length);
      ...
      var s2 = 0;
   		...
   		hghg2.value = jgdy2+'%';
   			//*******************线条****************//
     	var xt = document.getElementsByName('xt');
      var s2 = 0;
