    				...
    				<ul class="nav bs-docs-sidenav"> 					
    					...
    					<li><b style="margin-left:20px;">观感</b>
    						...
    						<ul class="nav">
    							...
    							<li class="active"><a href="guangan8.php?gcid=<?php echo $row["id"];?>&yhid=<?php echo $row["用户id"];?>&zljcid=<?php echo $_GET["zljcid"];?>&sjc=<?php echo $_GET["sjc"];?>">屋面及楼地面工程</a></li></br>
    						</ul>
    					</li>  
...
//										  $gcid1=$_GET["gcid"];
										...
										<!--<h3 class="panel-title label label-info"></h3>--><HR style="FILTER: progid:DXImageTransform.Microsoft.Shadow(color:#987cb9,direction:145,strength:15)" width="100%" color=#987cb9 SIZE=1>
												...
												<div class="form-group">
													...
													<div class="col-sm-3">
														...
														<input type="text" class="hidden" id="xmlb" name="xmlb" value="屋面及楼地面工程">
									 ...
									 ?>
										<div class="form-group">						
											...
											<?php
											  require("../conn.php");
											  $zljcid=$_GET["zljcid"];
												$sql = "select * from 测点布置附件表  where 质量检查id='$zljcid' and 实测类别='观感评价' and 项目类别='屋面及楼地面工程'  ";
												$result = $conn->query($sql);
												while($row = $result->fetch_assoc()) {
                      ?>
        ...
        <button type="button" class="btn btn-default"  onclick="javascript:window.open('cdbz/ggbz8.php?id=<?php echo $_GET["zljcid"];?>&xmlb=<?php echo '屋面及楼地面工程'?>&sclb=<?php echo '观感评价'?>', 'newwindow2', 'height=470, width=1100, top=100, left=200,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')">
        	测点布置
        </button>
			</div>
				<p></p>
		...
		<a href="#" id="myTabDrop1" class="dropdown-toggle" 
		   data-toggle="dropdown">测点类别筛选 <b class="caret"></b>
									...
									<ul class="dropdown-menu" role="menu" aria-labelledby="myTabDrop1" >
										...
										<li class=""><a href="#cd3"  tabindex="-1" data-toggle="tab">材料要求及普遍渗漏</a></li>
										</ul>
									<li>
									</ul>
			...
			<form id="cdbcform" name="cdbcform"  method="post" class="form-horizontal" role="form" >    							
                   ...
                   require("../conn.php");  
				   				 ...
				   				 $zljcid=$_GET["zljcid"]; 
								   	 ...
								   	 $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='屋面及楼地面工程' ";
            ...
            <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='屋面及楼地面工程' and 测点类别='工艺要求' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
              <div class="form-group" id="ceshiyong">
								...
								<label for="cdsl" class="col-sm-3 control-label">测点编号：</label>
								<div class="col-sm-2">
										<input type="text" class="form-control" readonly="readonly" id="c" name="" value="<?php echo $row["测点编号"];?>" >
								</div>
								<label for="cdsl" class="col-sm-3 control-label">观感评价：</label>
								<div class="col-sm-2" id="ceshila">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='屋面及楼地面工程' and 测点类别='观感质量' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
   <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='屋面及楼地面工程' and 测点类别='材料要求及普遍渗漏' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
      <div class="form-group" id="ceshiyong">
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='屋面及楼地面工程' and 测点类别 = '工艺要求' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='屋面及楼地面工程' and 测点类别 = '观感质量' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='屋面及楼地面工程' and 测点类别 = '材料要求及普遍渗漏' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
					...
					<div class="form-group col-sm-12" >
						<label for="cdlbA" class="col-sm-2 control-label">测点类别：</label>
						<div class="col-sm-2">
							<input type="text" class="form-control" id="cdlbA" name="cdlbA" value="材料要求及普遍渗漏" >
						</div>
						<label for="hgl" class="col-sm-2 control-label">合格率：</label>
						<div class="col-sm-2">
...
（不足10个以实计算）</textarea>	
							...
							<div class="col-sm-3">
											<input type="text" class="form-control hidden" id="xmlb" name="xmlb" value="屋面及楼地面工程" placeholder="">
												<input type="text" class="form-control hidden" id="xmlb" name="xmlb" value="<?php echo $_GET["gcid"];?>" placeholder="">
...
//				alert('12300');
			 ...
			 $("#cdlx1").click(function(){ 
			 	...
			 	if(cdlx1=="变形缝"){
			 		document.getElementById("nr").value='屋面及墙身的变形缝按要求封堵、设置砼挑板或盖板';
			 		$("#cdsl").val("10");
			 		
			 	}
			 	else if	(cdlx1=="檐口"){
			 		document.getElementById("nr").value='檐口按要求暗埋排水管';
			 		$("#cdsl").val("10");
			 		
			 	}
			 	else if	(cdlx1=="排水"){
			 		document.getElementById("nr").value='排水沟、平屋面排水坡向、坡度正确，按要求设置排水管';
			 		$("#cdsl").val("10");
			 		
			 	}
			 		else if	(cdlx1=="防水"){
			 		document.getElementById("nr").value='防水层施工前基底处理到位，防水材料厚度及施工工艺符合要求';
			 		$("#cdsl").val("10");
			 		
			 	}
			 		else if	(cdlx1=="分隔缝"){
			 		document.getElementById("nr").value='面层按要求设置分隔缝、嵌缝材料及灌缝质量符合要求';
			 		$("#cdsl").val("10");
			 ...
			 $("#cdlx2").click(function(){ 
			 	...
			 	var cdlx2=$("#cdlx2").val();
			 	if(cdlx2=="渗漏"){
			 		$("#nr").val('屋面、结构楼板无开裂、渗漏现象');
			 		$("#cdsl").val("10");
			 		
			 	}
			 	else if	(cdlx2=="空鼓开裂"){
			 		$("#nr").val('找平层、保护层按图施工，无开裂、空鼓、强度不足等');
			 		$("#cdsl").val("10");
			...
			$("#cdlx3").click(function(){
				...
				else if(cdlx3=='材料屋面防水施工前每户渗漏处数'){
					$("#nr").val('＞3处，超出部分每处扣0.5分（每户扣分不超过3分）');
					$("#cdsl").val("10");
				}
			});
</script>
