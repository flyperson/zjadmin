    				...
    				<ul class="nav bs-docs-sidenav"> 					
    					<li><b style="margin-left:20px;">实测</b>
    						...
    						<ul class="nav">
    							...
    							<li class="active"><a href="scsl3.php?gcid=<?php echo $row["id"];?>&yhid=<?php echo $row["用户id"];?>&zljcid=<?php echo $_GET["zljcid"];?>&sjc=<?php echo $_GET["sjc"];?>">设备安装工程</a></li>
    		...
    		<div class="col-md-10">
    			<div id="xmdj" class="panel panel-info">
						<div class="panel-heading">
							<h3 class="panel-title">设备安装工程</h3>
						</div>
...
//										  $gcid1=$_GET["gcid"];
										...
										<!--<h3 class="panel-title label label-info"></h3>--><HR style="FILTER: progid:DXImageTransform.Microsoft.Shadow(color:#987cb9,direction:145,strength:15)" width="100%" color=#987cb9 SIZE=1>
												...
												<div class="form-group">
													...
													<div class="col-sm-3">
														...
														<input type="text" class="hidden" id="xmlb" name="xmlb" value="设备安装工程">
									 ...
									 ?>
										<div class="form-group">						
											...
											<?php
											  require("../conn.php");
											  $zljcid=$_GET["zljcid"];
												$sql = "select * from 测点布置附件表  where 质量检查id='$zljcid' and 实测类别='实测实量' and 项目类别='设备安装工程' ";
												$result = $conn->query($sql);
												while($row = $result->fetch_assoc()) {
                      ?>
        ...
        <button type="button" class="btn btn-default"  onclick="javascript:window.open('cdbz/scbz3.php?id=<?php echo $_GET["zljcid"];?>&xmlb=<?php echo '设备安装工程' ?>&sclb=<?php echo '实测实量'?>', 'newwindow2', 'height=470, width=1100, top=100, left=200,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')">
        	测点布置                                                                                                                                                                                                                                   
        </button>
		...
		<a href="#" id="myTabDrop1" class="dropdown-toggle" 
		   data-toggle="dropdown">测点类别筛选 <b class="caret"></b>
									...
									<ul class="dropdown-menu" role="menu" aria-labelledby="myTabDrop1" >
										...
										<li class=""><a href="#cd1"  tabindex="-1" data-toggle="tab">座便坑距偏差</a></li>
									</ul>
			</div>
				<p></p>
				<div class="tab-content">
			...
			<form id="cdbcform" name="cdbcform"  method="post" class="form-horizontal" role="form" >    							
                   ...
                   require("../conn.php");  
				   				 ...
				   				 $zljcid=$_GET["zljcid"]; 
								   	 ...
								   	 $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='设备安装工程' ";
            ...
            <div class="form-group" id="ceshiyong">
                   ...
                   require("../conn.php");  
				   				 error_reporting(E_ALL ^ E_NOTICE);	 
				   				 $zljcid=$_GET["zljcid"]; 
								   $sql1 = "select * from  建筑工程质量检测表  where  检查项目='设备安装工程' and 检查内容='$xmlB' ";
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
	                   ...
	                   require("../conn.php");  
									   ...
									   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='设备安装工程' and 测点类别='座便坑距偏差' ";
	                   $result = $conn->query($sql);
	                   while($row = $result->fetch_assoc()) {
                   $xmlB = $row["测点类别"];
                   ?>
            <div class="form-group" id="ceshiyong">
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 测点实测值 from 实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='设备安装工程' and 测点类别 = '座便坑距偏差' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
					...
					<div class="form-group col-sm-12" >
						...
						<div class="col-sm-2">
							<input type="text" class="form-control" id="cdlbA" name="cdlbA" value="座便坑距偏差" >
						</div>
					...
					<div class="form-group">
						<label for="cdlb" class="col-sm-3 control-label">测点类别：</label>
								<div class="col-sm-3">
				                   ...
				                   require("../conn.php");   
								   				 $zljcid=$_GET["zljcid"];                
				                   $sql = "select distinct 检查内容 from  建筑工程质量检测表  where 检查项目='设备安装工程'";
				                   $result = $conn->query($sql);
				                   while($row = $result->fetch_assoc()) {
				                ?>
						...
						<div class="form-group ">
							...
							<div class="col-sm-3">
								<input type="text" class="form-control hidden" id="xmlb" name="xmlb" value="设备安装工程" placeholder="">
		...
		<script type="text/javascript">
			$("#save13").click(function(){ 
			 ...
			 $("#cdlb").click(function(){ 
			 	...
			 	if(cdlb=="座便坑距偏差"){
			 		document.getElementById("nr").value='同一户选取1个卫生间（1）每一个卫生间作为1 个实测区，累计实测实量5 个实测区。 （2）本指标在墙面打灰饼或抹灰完成或装饰面完成阶段，且管孔填嵌固定后测量。实测前，通过图纸确定座便器预留排水管孔距，并将其管孔中心距换算为管外壁距墙体装修完成面距离。如墙体装修面还未完成，现场测量值要减去2cm（墙面瓷砖铺贴预留厚度），以此作为偏差计算的数值进行合格性判断。每1个座便器预留排水管孔距的实测值与设计值之间的偏差值，作为判断该实测指标合格率的1个计算点。';
...
<script type="text/javascript">
 $("#cdlb").click(function(){ 
      ...
      var s = 0;
			for(var i=0;i<5;i++){
				if(cdsczpl[i].value<15 && cdsczpl[i].value>0){
					s=s+1;
				}
   	};
			var jgdy = s/5*100;
