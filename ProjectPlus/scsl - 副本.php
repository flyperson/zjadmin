...
//										  $gcid1=$_GET["gcid"];
		...
		<div role="tabpanel" class="tab-pane fade " id="scslsj">
	        ...
	        <button type="button" class="btn btn-default"  onclick="javascript:window.open('cdbzzz.php?id=<?php echo $_GET["zljcid"];?>&xmlb=<?php echo '混凝土结构工程' ?>&sclb=<?php echo '实测实量' ?>', 'newwindow2', 'height=420, width=315, top=0, left=500,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')">
	        	测点布置                                                                                                                                                                                                                                   
	        </button>
	        <button class="btn btn-default">
	        <form  method="post" class="form-horizontal">
	        <select id="cdlb1" name="cdlb1" class="form-control">
						<?php
				       require("../conn.php");                
				       $sql = "select distinct 检查内容   from  建筑工程质量检测表  where 检查项目='混凝土结构工程'";
				       $result = $conn->query($sql);
				       while($row = $result->fetch_assoc()) {
				    ?>
					...
					<input class="" id="sxzyc" name="sxzyc" value="" />
			...
			<form id="cdbcform" name="cdbcform"  method="post" class="form-horizontal" role="form" >    							
                   ...
                   require("../conn.php");  
				   				 ...
				   				 $zljcid=$_GET["zljcid"]; 
//								   $cdlb1=$_GET["cdlb1"];
								   $sxzyc = $_REQUEST['sxzyc']; 
								   if($sxzyc!=''){
								   	 $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='混凝土结构工程' ";
								   }
								   else{            
                   		$sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='混凝土结构工程' and 测点类别='$sxzyc' ";
									 } 
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>-->
	...
	<script type="text/javascript">
        $("#save3").click(function(){ 
			  ...
			  var cdlb1 = document.getElementById('cdlb1').value;
				...
				var sxzyc = document.getElementById('sxzyc');
				sxzyc.value = cdlb1;
...
//							return callback('ajax错误'+type+errorThrown);
    ...
    <script type="text/javascript">
			var creatDom=function(cdlb,cdbh,id,cdscz){
//				alert(cdlb+"**"+cdbh+"***"+id+"***"+cdscz);
//				alert ('99');
				var ceshiyong=document.getElementById("ceshiyong");
//				alert ('666');
				ceshiyong.innerHTML='<label for="scz" class="col-sm-2 control-label">测点类别：</label><div class="col-sm-2"><input type="text" class="form-control" id="scz" name="scz" value="'+cdlb+'" ><input type="text" class="form-control hidden" id="cddid" name="cddid" value="'+id+'" ></div><label for="cdsl" class="col-sm-2 control-label">测点编号：</label><div class="col-sm-2"><input type="text" class="form-control" id="cdsl" name="cdsl" value="'+cdbh+'" ></div><label for="cdsl" class="col-sm-2 control-label">测点实测值：</label><div class="col-sm-2" id="ceshila"><input type="text" class="form-control" id="" name="cdsl1" value="'+cdscz+'" placeholder="测点实测值"><input type="text" class="form-control hidden" id="" name="cdsl2" value="'+id+'" ></div>';
//				alert ('332');
			};
	
			var cdlb1=document.getElementById("cdlb1");
			cdlb1.addEventListener('change',function getData(){
				//alert(dept1.value);
//				var ceshiyong=document.getElementById("ceshiyong");
				var zljcid=document.getElementById("zljcid").value;
//				ceshiyong.innerHTML="";
				//creatDom(cdlb,cdbh,cdscz);
				$.ajax({
						 url:'cddtcj.php',
						data:{
							zljcid:zljcid
...
//							alert(data);
							...
							if(length<0){
								creatDom('scz','cdsl','id','cdscz');
							}else{
								for (var i=0;i<length-1;i++) {
									var scz=data[i].测点类别;
									var cdsl=data[i+1].测点编号;
									var id=data[i+2].id;
									var cdscz=data[i+3].测点实测值;
									creatDom(scz,cdsl,id,cdscz);
								}
							}
