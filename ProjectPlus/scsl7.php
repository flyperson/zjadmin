    				...
    				<ul class="nav bs-docs-sidenav"> 					
    					...
    					<li>
    						...
    						<ul class="nav">
    							...
    							<li class="active"><a href="scsl7.php?gcid=<?php echo $row["id"];?>&yhid=<?php echo $row["用户id"];?>&zljcid=<?php echo $_GET["zljcid"];?>&sjc=<?php echo $_GET["sjc"];?>">电梯前室、首层大堂</a></li>
    		...
    		<div class="col-md-10">
    			<div id="xmdj" class="panel panel-info">
						<div class="panel-heading">
							<h3 class="panel-title">电梯前室、首层大堂</h3>
						</div>
...
//										  $gcid1=$_GET["gcid"];
										...
										<!--<h3 class="panel-title label label-info"></h3>--><HR style="FILTER: progid:DXImageTransform.Microsoft.Shadow(color:#987cb9,direction:145,strength:15)" width="100%" color=#987cb9 SIZE=1>
												...
												<div class="form-group">
													...
													<div class="col-sm-3">
														...
														<input type="text" class="hidden" id="xmlb" name="xmlb" value="电梯前室、首层大堂">
									 ...
									 ?>
										<div class="form-group">						
											...
											<?php
											  require("../conn.php");
											  $zljcid=$_GET["zljcid"];
												$sql = "select * from 测点布置附件表  where 质量检查id='$zljcid' and 实测类别='实测实量' and 项目类别='电梯前室、首层大堂' ";
												$result = $conn->query($sql);
												while($row = $result->fetch_assoc()) {
                      ?>
        ...
        <button type="button" class="btn btn-default"  onclick="javascript:window.open('cdbz/scbz7.php?id=<?php echo $_GET["zljcid"];?>&xmlb=<?php echo '电梯前室、首层大堂' ?>&sclb=<?php echo '实测实量' ?>', 'newwindow2', 'height=470, width=1100, top=100, left=200,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')">
        	测点布置                                                                                                                                                                                                                                   
        </button>
		...
		<a href="#" id="myTabDrop1" class="dropdown-toggle" 
		   data-toggle="dropdown">测点类别筛选 <b class="caret"></b>
									...
									<ul class="dropdown-menu" role="menu" aria-labelledby="myTabDrop1" >
										...
										<li class=""><a href="#cd1"  tabindex="-1" data-toggle="tab">墙面砖垂直度</a></li>
										<li class=""><a href="#cd2"  tabindex="-1" data-toggle="tab">墙面砖接缝高低差</a></li>
											<li class=""><a href="#cd3"  tabindex="-1" data-toggle="tab">地面砖表面平整度</a></li>
										<li class=""><a href="#cd4"  tabindex="-1" data-toggle="tab">地面砖接缝高低差</a></li>
			...
			<form id="cdbcform" name="cdbcform"  method="post" class="form-horizontal" role="form" >    							
                   ...
                   require("../conn.php");  
				   				 ...
				   				 $zljcid=$_GET["zljcid"]; 
								   	$sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='电梯前室、首层大堂' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                  $xmlB = $row["测点类别"];
                   ?>
            <div class="form-group" id="ceshiyong">
                   ...
                   require("../conn.php");  
				   				 error_reporting(E_ALL ^ E_NOTICE);	 
				   				 $zljcid=$_GET["zljcid"]; 
								   $sql1 = "select * from  装修工程质量检查表  where  检查项目='电梯前室、首层大堂' and 检查内容='$xmlB' ";
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='电梯前室、首层大堂' and 测点类别 LIKE '%墙面砖垂直度%' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   $xmlB = $row["测点类别"];
                   ?>
            <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='电梯前室、首层大堂' and 测点类别='墙面砖接缝高低差' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   $xmlB = $row["测点类别"];
                   ?>
            <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='电梯前室、首层大堂' and 测点类别='地面砖表面平整度' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   $xmlB = $row["测点类别"];
                   ?>
            <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='电梯前室、首层大堂' and 测点类别='地面砖接缝高低差' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   $xmlB = $row["测点类别"];
                   ?>
			...
			<div role="tabpanel" class="tab-pane fade " id="tjfx">
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 测点实测值 from 实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='电梯前室、首层大堂' and 测点类别 = '墙面砖垂直度' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 测点实测值 from 实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='电梯前室、首层大堂' and 测点类别 = '墙面砖接缝高低差' ";      
	         ...
	         ?>
					...
					<?php
	           require("../conn.php");  
             $sql = "select 测点实测值 from 实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='电梯前室、首层大堂' and 测点类别 = '地面砖表面平整度'";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
					...
					<div class="form-group col-sm-12" >
						...
						<div class="col-sm-2">
							<input type="text" class="form-control" id="cdlbA" name="cdlbA" value="墙面砖垂直度" >
						</div>
					...
					<div class="form-group col-sm-12" >
						...
						<div class="col-sm-2">
							<input type="text" class="form-control" id="qbmpzd" name="qbmpzd" value="墙面砖接缝高低差" >
						</div>
					...
					<div class="form-group col-sm-12" >
						...
						<div class="col-sm-2">
							<input type="text" class="form-control" id="czdmm" name="czdmm" value="地面砖表面平整度" >
						</div>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 测点实测值 from 实测实量新建测点 where 质量检查id='$zljcid' and 项目类别='电梯前室、首层大堂' and 测点类别 = '地面砖接缝高低差' ";      
	         ...
	         ?>
					...
					<div class="form-group col-sm-12" >
						<label for="qbmpzd" class="col-sm-2 control-label">测点类别：</label>
						<div class="col-sm-2">
							<input type="text" class="form-control" id="qbmpzd" name="qbmpzd" value="地面砖接缝高低差" >
						</div>
					...
					<div class="form-group">
						<label for="cdlb" class="col-sm-3 control-label">测点类别：</label>
								<div class="col-sm-3">
				                   ...
				                   require("../conn.php");   
								   				 $zljcid=$_GET["zljcid"];                
				                   $sql = "select distinct 检查内容 from  装修工程质量检查表  where 检查项目='电梯前室、首层大堂'";
				                   $result = $conn->query($sql);
				                   while($row = $result->fetch_assoc()) {
				                ?>
						...
						<div class="form-group ">
							...
							<div class="col-sm-3">
								<input type="text" class="form-control hidden" id="xmlb" name="xmlb" value="电梯前室、首层大堂" placeholder="">
...
<script type="text/javascript">
 $("#cdlb").click(function(){ 
			 	...
			 	if(cdlb=="墙面砖垂直度(瓷砖墙面)"){
			 		document.getElementById("nr").value='同一电梯前室、首层大堂选取2个墙面（电梯门两侧）（1）每一面墙都作为1个实测区，累计实测实量10 个实测区。如该电梯前室、首层大堂内还有石材或木质墙面，则各增加5个实测区。（2）同一面墙距两端头竖向阴阳角约30cm位置，分别按以下原则实测2次：一是靠尺顶端接触到上部砼顶板位置时测1次垂直度，二是靠尺底端接触到下部地面位置时测1次垂直度；电梯门及消防门两侧为必测项。（3）以上各实测值作为合格率1个计算点。';
			 		$("#cdsl").val("20");
			 		
			 		
			 	}
			else 	if(cdlb=="墙面砖垂直度(石材墙面)"){
			 		document.getElementById("nr").value='同一电梯前室、首层大堂选取2个墙面（电梯门两侧）（1）每一面墙都作为1个实测区，累计实测实量10 个实测区。如该电梯前室、首层大堂内还有石材或木质墙面，则各增加5个实测区。（2）同一面墙距两端头竖向阴阳角约30cm位置，分别按以下原则实测2次：一是靠尺顶端接触到上部砼顶板位置时测1次垂直度，二是靠尺底端接触到下部地面位置时测1次垂直度；电梯门及消防门两侧为必测项。（3）以上各实测值作为合格率1个计算点。';
			 		$("#cdsl").val("10");
			 		
			 		
			 	}
			else 	if(cdlb=="墙面砖垂直度(木质墙面)"){
			 		document.getElementById("nr").value='同一电梯前室、首层大堂选取2个墙面（电梯门两侧）（1）每一面墙都作为1个实测区，累计实测实量10 个实测区。如该电梯前室、首层大堂内还有石材或木质墙面，则各增加5个实测区。（2）同一面墙距两端头竖向阴阳角约30cm位置，分别按以下原则实测2次：一是靠尺顶端接触到上部砼顶板位置时测1次垂直度，二是靠尺底端接触到下部地面位置时测1次垂直度；电梯门及消防门两侧为必测项。（3）以上各实测值作为合格率1个计算点。';
			 		$("#cdsl").val("10");
			 		
			 		
			 	}
			 	else if	(cdlb=="墙面砖接缝高低差"){
			 		document.getElementById("nr").value='同一电梯前室、首层大堂选取2个墙面（任意两面墙）（1）该指标宜在装修收尾阶段测量。每一墙面作为1个实测区，累计实测实量10 个测区。（2） 在每一饰面砖墙面，目测选取2 条疑似高低差最大的饰面砖接缝。用钢尺或其他辅助工具紧靠相邻两饰面砖跨过接缝，用0.5mm 钢塞片插入钢尺与饰面砖之间的缝隙。如能插入，则该测量点不合格；反之则该测量点合格。2 条接缝高低差的实测值，分别作为判断该实测指标合格率的2 个计算点。（3） 为数据统计方便和提高实测效率，不合格点均按0.7mm 记录，合格点均按0.3mm记录。';
			 		$("#cdsl").val("20");
			 		
			 		
			 	}
			 	else if	(cdlb=="地面砖表面平整度"){
			 		document.getElementById("nr").value='同一电梯前室、首层大堂为一个测区（1）每一电梯前室、首层大堂饰面砖地面都可以作为1个实测区，累计实测实量5 个实测区。如还有石材地面，则增加5个实测区。（2）每一测区任选3个部位平放靠尺共测量3次，这3个实测值作为判断该实测指标合格率的3个计算点。';
			 		$("#cdsl").val("15");
			 		
			 		
			 		
			 	}
			 	else if	(cdlb=="地面砖接缝高低差"){
			 		document.getElementById("nr").value='同一电梯前室、首层大堂为一个测区（1）每一电梯前室、首层大堂饰面砖地面作为一个实测区，累计实测实量5 个测区。（2）每个测区任意选择4个检测点。（3）用钢尺或其他辅助工具紧靠相邻两饰面砖跨过接缝，用0.5MM钢塞片插入钢尺与饰面砖之间的缝隙。如能插入，则该测量点不合格；反之则该测量点合格。每条接缝高低差的测量值，均作为判断该实测指标合格率的1个计算点。（4） 为数据统计方便和提高实测效率，不合格点均按0.7mm 记录，合格点均按0.3mm记录。';
			 		$("#cdsl").val("20");
      ...
      var s = 0;
			for(var i=0;i<20;i++){
				if(cdsczpl[i].value<2 && cdsczpl[i].value>0){
					s=s+1;
				}
   	};
			var jgdy = s/20*100;
...
//			************墙表面平整度********************//
      ...
      var s1 = 0;
			for(var i=0;i<20;i++){
				if(qmpzd[i].value<0.5 && qmpzd[i].value>0){
					s1=s1+1;
				}
				else{
					s1 =s1;
...
//			************垂直度（免抹灰）********************//
      ...
      var s2 = 0;
			for(var i=0;i<15;i++){
				if(czdmmh[i].value<2 && czdmmh[i].value>0){
					s2=s2+1;
				}
   	};
			var jgdy2 = s2/15*100;
      ...
      var s3 = 0;
			for(var i=0;i<20;i++){
				if(qmpzd1[i].value<0.5 && qmpzd[i].value>0){
					s3=s3+1;
				}
				else{
					s3 =s3;
   		...
   		hgl3.value = jgdy3+'%';	
   		//***********************算本项目合格率*****************//  
   		var hnthelpj =(jgdy+jgdy1+jgdy2+jgdy3)/4+'%';
