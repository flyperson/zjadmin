    			...
    			<div class="bs-docs-sidebar affix" role="complementary">
    				<ul class="nav bs-docs-sidenav"> 					
    					...
    					<li>
    						...
    						<ul class="nav">
    							<li class="active"><a href="guangan9.php?gcid=<?php echo $row["id"];?>&yhid=<?php echo $row["用户id"];?>&zljcid=<?php echo $_GET["zljcid"];?>&sjc=<?php echo $_GET["sjc"];?>">外墙工程</a></li>
...
//										  $gcid1=$_GET["gcid"];
										...
										<!--<h3 class="panel-title label label-info"></h3>--><HR style="FILTER: progid:DXImageTransform.Microsoft.Shadow(color:#987cb9,direction:145,strength:15)" width="100%" color=#987cb9 SIZE=1>
												...
												<div class="form-group">
													...
													<div class="col-sm-3">
														...
														<input type="text" class="hidden" id="xmlb" name="xmlb" value="外墙工程">
									 ...
									 ?>
										<div class="form-group">						
											...
											<?php
											  require("../conn.php");
											  $zljcid=$_GET["zljcid"];
												$sql = "select * from 测点布置附件表  where 质量检查id='$zljcid' and 实测类别='观感评价' and 项目类别='外墙工程'  ";
												$result = $conn->query($sql);
												while($row = $result->fetch_assoc()) {
                      ?>
        ...
        <button type="button" class="btn btn-default"  onclick="javascript:window.open('cdbz/ggbz9.php?id=<?php echo $_GET["zljcid"];?>&xmlb=<?php echo '外墙工程'?>&sclb=<?php echo '观感评价'?>', 'newwindow2', 'height=420, width=315, top=0, left=500,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')">
        	测点布置
        </button>
			</div>
				<p></p>
		...
		<a href="#" id="myTabDrop1" class="dropdown-toggle" 
		   data-toggle="dropdown">测点类别筛选 <b class="caret"></b>
									...
									<ul class="dropdown-menu" role="menu" aria-labelledby="myTabDrop1" >
										...
										<li class=""><a href="#cd1"  tabindex="-1" data-toggle="tab">外墙铺贴</a></li>
										<li class=""><a href="#cd2"  tabindex="-1" data-toggle="tab">外墙涂料</a></li>
										<li class=""><a href="#cd3"  tabindex="-1" data-toggle="tab">外墙石材</a></li>
										</ul>
									<li>
									</ul>
			...
			<form id="cdbcform" name="cdbcform"  method="post" class="form-horizontal" role="form" >    							
                   ...
                   require("../conn.php");  
				   				 ...
				   				 $zljcid=$_GET["zljcid"]; 
								   	 ...
								   	 $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='外墙工程' ";
             ...
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='外墙工程' and 测点类别='外墙铺贴' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='外墙工程' and 测点类别='外墙涂料' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='外墙工程' and 测点类别='外墙石材' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
              <div class="form-group" id="ceshiyong">
			...
			<div role="tabpanel" class="tab-pane fade " id="tjfx">
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='外墙工程' and 测点类别 = '外墙铺贴' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='外墙工程' and 测点类别 = '外墙涂料' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='外墙工程' and 测点类别 = '外墙石材' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
					...
					<div class="form-group col-sm-12" >
						...
						<div class="col-sm-2">
							<input type="text" class="form-control" id="cdlbA" name="cdlbA" value="外墙铺贴" >
						</div>
					...
					<div class="form-group col-sm-12" >
						...
						<div class="col-sm-2">
							<input type="text" class="form-control" id="qbmpzd" name="qbmpzd" value="外墙涂料" >
						</div>
					...
					<div class="form-group col-sm-12" >
						<label for="cdlbA" class="col-sm-2 control-label">测点类别：</label>
						<div class="col-sm-2">
							<input type="text" class="form-control" id="cdlbA" name="cdlbA" value="外墙石材" >
						</div>
						<label for="hgl" class="col-sm-2 control-label">合格率：</label>
						<div class="col-sm-2">
			...
			<div class="modal fade" id="myModal1" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
					...
					<div class="form-group">
						...
						<div class="form-group ">	
								...
								<div class="col-sm-9">
									<textarea class="form-control" rows="6" id="cq"  name="cq">抽检10户中的外墙可视区作为50个测区，涵盖首层入户位置、各个阳露台、外窗、屋面的可视范围，每一个可视范围作为一个测区
（不足50个以实计算）注：检查需包含所有材质</textarea>	
								</div>	
							</div>
									<div class="form-group ">
							...
							<div class="col-sm-3">
											<input type="text" class="form-control hidden" id="xmlb" name="xmlb" value="外墙工程" placeholder="">
...
//							return callback('ajax错误'+type+errorThrown);
			 ...
			 $("#cdlx1").click(function(){ 
			 	...
			 	if(cdlx1=="图案"){
			 		document.getElementById("nr").value='铺贴图案符合图纸要求，整齐美观'
			 		
			 	}
			 	else if	(cdlx1=="表面"){
			 		document.getElementById("nr").value='平整，无色差，无污染，缝隙均匀、饱满'
			 		
			 	}
			 	
			 	 	else if	(cdlx1=="收口"){
			 		document.getElementById("nr").value='边角收口好、无崩损、无空鼓或脱落'
			 		
			 	}
			 	
			 	 else if (cdlx1="线条"){
			 	 	document.getElementById("nr").value='线条饰件顺直美观；滴水线、鹰嘴按要求设置 '
			 	 }
			 ...
			 $("#cdlx2").click(function(){ 
			 	...
			 	if(cdlx2=="基层"){
			 		...
			 		$("#nr").val('平整，无：色差、污染、返碱、咬色、开裂、破损')
			 	}
			 	else if(cdlx2=="线条"){
			 		$("#nr").val('线条饰件顺直美观；滴水线、鹰嘴按要求设置 ')
			 	}
			...
			$("#cdlx3").click(function(){
				...
				if(cdlx3=="安装"){
					...
					$("#nr").val('平整、对缝，无崩角，无返锈、无严重色差，无污染')
				}
				else if(cdlx3=="线条"){
					$("#nr").val('拼接、打磨效果好')
				}
				else if(cdlx3=="打胶"){
					$("#nr").val('缝宽均匀、顺直、无大小头、无错缝')
				}
			});
</script>
  </body>
