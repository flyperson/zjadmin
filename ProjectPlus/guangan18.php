...
//										echo $_GET["id"];
    				...
    				<ul class="nav bs-docs-sidenav"> 					
    					...
    					<li>
    						...
    						<ul class="nav">
    							...
    							<li class="active"><a href="guangan18.php?gcid=<?php echo $row["id"];?>&yhid=<?php echo $row["用户id"];?>&zljcid=<?php echo $_GET["zljcid"];?>&sjc=<?php echo $_GET["sjc"];?>">门窗细部工程</a></li>
	...
	<div class="panel-heading">
		<h3 class="panel-title">门窗细部工程</h3>
	</div>
...
//										  $gcid1=$_GET["gcid"];
										...
										<!--<h3 class="panel-title label label-info"></h3>--><HR style="FILTER: progid:DXImageTransform.Microsoft.Shadow(color:#987cb9,direction:145,strength:15)" width="100%" color=#987cb9 SIZE=1>
												...
												<div class="form-group">
													...
													<div class="col-sm-3">
														...
														<input type="text" class="hidden" id="xmlb" name="xmlb" value="门窗细部工程">
									 ...
									 ?>
										<div class="form-group">						
											...
											<?php
											  require("../conn.php");
											  $zljcid=$_GET["zljcid"];
												$sql = "select * from 测点布置附件表  where 质量检查id='$zljcid' and 实测类别='观感评价' and 项目类别='门窗细部工程'  ";
												$result = $conn->query($sql);
												while($row = $result->fetch_assoc()) {
                      ?>
        ...
        <button type="button" class="btn btn-default"  onclick="javascript:window.open('cdbz/ggbz18.php?id=<?php echo $_GET["zljcid"];?>&xmlb=<?php echo '门窗细部工程'?>&sclb=<?php echo '观感评价'?>', 'newwindow2', 'height=420, width=315, top=0, left=500,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')">
        	测点布置
        </button>
			</div>
				<p></p>
		...
		<a href="#" id="myTabDrop1" class="dropdown-toggle" 
		   data-toggle="dropdown">测点类别筛选 <b class="caret"></b>
									...
									<ul class="dropdown-menu" role="menu" aria-labelledby="myTabDrop1" >
										...
										<li class=""><a href="#cd1"  tabindex="-1" data-toggle="tab">木门 </a></li>
										
										<li class=""><a href="#cd2"  tabindex="-1" data-toggle="tab">铝合金、塑钢门、窗 </a></li>
									</ul>	
									<li>
									</ul>
			...
			<form id="cdbcform" name="cdbcform"  method="post" class="form-horizontal" role="form" >    							
                   ...
                   require("../conn.php");  
				   				 ...
				   				 $zljcid=$_GET["zljcid"]; 
								   	 ...
								   	 $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='门窗细部工程' ";
             ...
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='门窗细部工程' and 测点类别='木门' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
             <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='门窗细部工程' and 测点类别='铝合金、塑钢门、窗' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
             <div class="form-group" id="ceshiyong">
			...
			<div role="tabpanel" class="tab-pane fade " id="tjfx">
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='门窗细部工程' and 测点类别 = '木门' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='门窗细部工程' and 测点类别 = '铝合金、塑钢门、窗' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
					...
					<div class="form-group col-sm-12" >
						...
						<div class="col-sm-2">
							<input type="text" class="form-control" id="cdlbA" name="cdlbA" value="木门" >
						</div>
					...
					<div class="form-group col-sm-12" >
						...
						<div class="col-sm-2">
							<input type="text" class="form-control" id="qbmpzd" name="qbmpzd" value="铝合金、塑钢门、窗" >
						</div>
			...
			<div class="modal fade" id="myModal1" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
					...
					<div class="form-group">
						...
						<label for="cdlx1" class="col-sm-3 control-label">测点类型：</label>
										<div class="col-sm-3">
											<select id="cdlx1" name="cdlx1" class="form-control">
							                   ...
							                   require("../conn.php");  
							   				  $zljcid=$_GET["zljcid"]; 
											   $sql = "select 测点类型 from  观感评价测点类型表 where  工程类别='门窗细部工程'and 测点类别='木门'  ";
							                   $result = $conn->query($sql);
								                   while($row = $result->fetch_assoc()) {
								                   ?>
											...
											<select id="cdlx2" name="cdlx2" class="form-control hidden">
							                   ...
							                   require("../conn.php");  
							   				  $zljcid=$_GET["zljcid"]; 
											   $sql = "select 测点类型 from  观感评价测点类型表 where  工程类别='门窗细部工程'and 测点类别='铝合金、塑钢门、窗'  ";
							                   $result = $conn->query($sql);
								                   while($row = $result->fetch_assoc()) {
								                   ?>
												<option><?php echo $row["测点类型"];?></option>
						...
						<div class="form-group ">	
								...
								<div class="col-sm-9">
									<textarea class="form-control" rows="6" id="cq"  name="cq"></textarea>	
								</div>	
							</div>
									<div class="form-group ">
							...
							<div class="col-sm-3">
											<input type="text" class="form-control hidden" id="xmlb" name="xmlb" value="门窗细部工程" placeholder="">
	      ...
	      $('#cdlx1').addClass("hidden");
		    $('#cdlx2').addClass("hidden");	
				...
				$("#cdlx1").click(function(){ 
			 	...
			 	if(cdlx1=="安装"){
			 		document.getElementById("nr").value='门套与墙体间发泡胶施打符合要求'
			 		
			 	}
			 	else if	(cdlx1=="门"){
			 		document.getElementById("nr").value='框安装牢固，水平，开关灵活、严密，表面清洁、无划伤等现象，门扇不变形，门扇与门框缝隙均匀，木门倒角顺直、严密'
			 ...
			 $("#cdlx2").click(function(){ 
			 	...
			 	if(cdlx2=="保护膜"){
			 		...
			 		$("#nr").val('窗框设置排水孔，位置和数量符合设计要求')
			 		
			 	}
			 	else if(cdlx2=="打胶"){
			 		$("#nr").val('窗框、窗扇按要求打胶')
			 	}
			 	else if	(cdlx2=="门扇离地缝隙"){
			 		$("#nr").val('门扇离地缝隙合理')
			 		
			 	}
			 	else if(cdlx2=="安装平正"){
			 		$("#nr").val('安装平正牢固，开关灵活，收口美观、胶颜色与型材相同，打胶顺直')
			 	}
			});
			 $("#cdlb").click(function(){ 
			 	...
			 	if(cdlb=="木门"){
			 		document.getElementById("cq").value='抽检10户中的30扇门作为30个测区（不足30个以实计算）'
			 ...
			 var cdlb=document.getElementById("cdlb").value;
			 	if(cdlb=="铝合金、塑钢门、窗"){
			 		document.getElementById("cq").value='抽检10户中的30个门或窗作为30个测区（不足30个以实计算）'
