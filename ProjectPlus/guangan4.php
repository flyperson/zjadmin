...
//										echo $_GET["id"];
    				...
    				<ul class="nav bs-docs-sidenav"> 					
    					...
    					<li><b style="margin-left:20px;">观感</b>
    						...
    						<ul class="nav">
    							...
    							<li class="active"><a href="guangan4.php?gcid=<?php echo $row["id"];?>&yhid=<?php echo $row["用户id"];?>&zljcid=<?php echo $_GET["zljcid"];?>&sjc=<?php echo $_GET["sjc"];?>">外墙防水工程</a></li>
	...
	<div class="panel-heading">
		<h3 class="panel-title">外墙防水工程</h3>
	</div>
...
//										  $gcid1=$_GET["gcid"];
										...
										<!--<h3 class="panel-title label label-info"></h3>--><HR style="FILTER: progid:DXImageTransform.Microsoft.Shadow(color:#987cb9,direction:145,strength:15)" width="100%" color=#987cb9 SIZE=1>
												...
												<div class="form-group">
													...
													<div class="col-sm-3">
														...
														<input type="text" class="hidden" id="xmlb" name="xmlb" value="外墙防水工程">
									 ...
									 ?>
										<div class="form-group">						
											...
											<?php
											  require("../conn.php");
											  $zljcid=$_GET["zljcid"];
												$sql = "select * from 测点布置附件表  where 质量检查id='$zljcid' and 实测类别='观感评价' and 项目类别='外墙防水工程'  ";
												$result = $conn->query($sql);
												while($row = $result->fetch_assoc()) {
                      ?>
        ...
        <button type="button" class="btn btn-default"  onclick="javascript:window.open('cdbz/ggbz4.php?id=<?php echo $_GET["zljcid"];?>&xmlb=<?php echo '外墙防水工程'?>&sclb=<?php echo '观感评价'?>', 'newwindow2', 'height=470, width=1100, top=100, left=200,toolbar=no, menubar=no, scrollbars=no, resizable=no, location=no, status=no')">
        	测点布置
        </button>
			</div>
				<p></p>
		...
		<a href="#" id="myTabDrop1" class="dropdown-toggle" 
		   data-toggle="dropdown">测点类别筛选 <b class="caret"></b>
									...
									<ul class="dropdown-menu" role="menu" aria-labelledby="myTabDrop1" >
										...
										<li class=""><a href="#cd3"  tabindex="-1" data-toggle="tab">材料要求</a></li>
										</ul>
									<li>
									</ul>
			...
			<form id="cdbcform" name="cdbcform"  method="post" class="form-horizontal" role="form" >    							
                   ...
                   require("../conn.php");  
				   				 ...
				   				 $zljcid=$_GET["zljcid"]; 
								   	 ...
								   	 $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='外墙防水工程' ";
              ...
              <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='外墙防水工程' and 测点类别='工艺要求' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
              <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='外墙防水工程' and 测点类别='观感质量' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
              <div class="form-group" id="ceshiyong">
				...
				<form id="cdbcform2" name="cdbcform"  method="post" class="form-horizontal active" role="form" >    							
                   ...
                   require("../conn.php");  
								   ...
								   $sql = "select * from  观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='外墙防水工程' and 测点类别='材料要求' ";
                   $result = $conn->query($sql);
                   while($row = $result->fetch_assoc()) {
                   ?>
              <div class="form-group" id="ceshiyong">
		...
		<div role="tabpanel" class="tab-pane fade " id="tjfx">
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='外墙防水工程' and 测点类别 = '工艺要求' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='外墙防水工程' and 测点类别 = '观感质量' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
	           ...
	           require("../conn.php");  
			   		 $zljcid=$_GET["zljcid"]; 
             $sql = "select 观感评价 from 观感评价新建测点 where 质量检查id='$zljcid' and 项目类别='外墙防水工程' and 测点类别 = '材料要求' ";      
	           $result = $conn->query($sql);
	           while($row = $result->fetch_assoc()) {
	         ?>
					...
					<div class="form-group col-sm-12" >
						<label for="cdlbA" class="col-sm-2 control-label">测点类别：</label>
						<div class="col-sm-2">
							<input type="text" class="form-control" id="cdlbA" name="cdlbA" value="材料要求" >
						</div>
						<label for="hgl" class="col-sm-2 control-label">合格率：</label>
						<div class="col-sm-2">
						...
						<label for="cdlx" class="col-sm-3 control-label">测点类型：</label>
										<div class="col-sm-3">
											<select id="cdlx1" name="cdlx1" class="form-control">
												<option>基底、拉毛、挂网</option>
												<option>抹灰、防水、保温层工艺</option>
												<option>拦河、压顶、外墙线条</option>
												<option>分格缝、滴水线</option>
											</select>
											<select id="cdlx2" name="cdlx2" class="form-control hidden">
												<option>抹灰、防水、保温层观感</option>
												<option>檐口、歇山</option>
												<option>渗漏</option>	
											</select>
...
（不足10个以实计算）</textarea>	
							...
							<div class="col-sm-3">
											<input type="text" class="form-control hidden" id="xmlb" name="xmlb" value="外墙防水工程" placeholder="">
...
//				alert('12300');
			...
			$("#cdlx1").click(function(){ 
			 	...
			 	if(cdlx1=="基底、拉毛、挂网"){
			 		document.getElementById("nr").value='拉毛、挂网前基底已按要求处理，外墙孔洞（支模孔洞、对拉螺栓、脚手架预留洞、施工洞口等）按要求封堵；不同材料交接处按要求挂钢丝网；拉毛粘结牢固、毛刺感强，密度、强度满足要求';
			 		$("#cdsl").val("10");
			 		
			 	}
			 	else if	(cdlx1=="抹灰、防水、保温层工艺"){
			 		document.getElementById("nr").value='按要求分层抹灰；抹灰总厚度≥35mm时，中间增设加强网片；外墙满挂钢丝网且放于抹灰中间层，钢丝网平整绷紧；防水层按设计要求施工，防水层厚度符合要求，防水层在门窗洞口、穿墙管、预埋件、分格缝及收头等细部节点做法，符合设计要求；防水层完成后打凿破坏及时修复；保温层按设计要求施工，保温砂浆厚度符合要求，保温板按要求粘贴及固定；外墙构件（栏杆、铁艺等）、幕墙或石材预埋件，在保温层施工前安装完成；严禁保温层完成后，再进行剔凿施工';
			 		$("#cdsl").val("10");
			 		
			 		
			 	}
			 	if(cdlx1=="拦河、压顶、外墙线条"){
			 		document.getElementById("nr").value='阳露台拦河、女儿墙压顶放坡正确；线条施工符合要求；水平线条顶面粉刷坡度大于5%；成品饰线在防水砂浆层完成后安装，且连接牢固、接缝打胶饱满；严禁在砖砌体上植筋现浇砼外墙饰线';
			 		$("#cdsl").val("10");
			 		
			 	}
			 	if(cdlx1=="分格缝、滴水线"){
			 		document.getElementById("nr").value='外墙按要求留设分格缝，严禁后期切缝；分格缝填缝符合要求；挑板、挑檐、阳台、雨篷、檐口、门窗楣、外窗台、门廊等底部和突出外墙的线条按要求设置滴水线、滴水槽、鹰嘴';
			 		$("#cdsl").val("10");
			 ...
			 $("#cdlx2").click(function(){ 
			 	...
			 	var cdlx2=$("#cdlx2").val();
			 	if(cdlx2=="抹灰、防水、保温层观感"){
			 		$("#nr").val('抹灰层无开裂、露网；防水层表面应密实、平整，不得有裂纹、起砂、麻面等缺陷；保温层无脱层，面层无粉化、起皮、爆灰等缺陷');
			 		$("#cdsl").val("10");
			 		
			 	}
			 	else if	(cdlx2=="檐口、歇山"){
			 		$("#nr").val('坡屋面檐口、歇山无通长裂缝或泛碱');
			 		$("#cdsl").val("10");
