...
//										echo $_GET["id"];
    			...
    			<div id="xmdj" class="panel panel-info">
					...
					<div class="panel-body">
					  <div class="form-group col-sm-12" >
						<label for="jzdfjg" class="col-sm-3 control-label">建筑工程得分：</label>
							<div class="col-sm-3">
								<input type="text" class="form-control" id="jzdfjg" name="jzdfjg" value="" >
							</div>
						<label for="zsdfjg" class="col-sm-3 control-label">装饰工程得分：</label>
							<div class="col-sm-3">
								<input type="text" class="form-control" id="zsdfjg" name="zsdfjg" value="" >
							</div>
					  </div>
					   <div class="form-group col-sm-12" >
						<label for="gczdf" class="col-sm-3 control-label">工程总得分：</label>
							<div class="col-sm-3">
								<input type="text" class="form-control" id="gczdf" name="gczdf" value="" >
							</div>
					  </div>
						<?php
						  $zljcid=$_GET["zljcid"];
						  require("../conn.php");
						  $sql = "select 合格率 from 统计  where 质量检查id='$zljcid' and  项目类别 like  '%建筑%'";
						  $result = $conn->query($sql);
						  while($row = $result->fetch_assoc()) {
		                ?>
		                <input name="jzdhgl" class="hidden" value="<?php echo $row["合格率"];?>" />
		                <?php
							}
							$conn->close();											
						?>
						<?php
						  $zljcid=$_GET["zljcid"];
						  require("../conn.php");
						  $sql = "select 合格率 from 统计  where 质量检查id='$zljcid' and  项目类别 like  '%装饰%'";
						  $result = $conn->query($sql);
						  while($row = $result->fetch_assoc()) {
		                ?>
		                <input name="zsdhgl" class="hidden" value="<?php echo $row["合格率"];?>" />
		                <?php
							}
							$conn->close();											
						?>
   ...
   <script src="../js/bootstrap-table-zh-CN.min.js"></script>
	...
	<script>
		var jzdhgl = document.getElementsByName('jzdhgl');
		var jzdfjg1 = 0;
		var zsdhgl = document.getElementsByName('zsdhgl');
		var zsdfjg1 = 0;
//		alert(jzdhgl.length);
		for (var i=0;i<jzdhgl.length;i++){
			strs=jzdhgl[i].value.split("%");
			var aaaddd = parseInt(strs);
			jzdfjg1 =(jzdfjg1+aaaddd)/jzdhgl.length;
//			jzdfjg1 =eval(jzdfjg1+strs);
		}
		for (var j=0;j<zsdhgl.length;j++){
			strs1=zsdhgl[j].value.split("%");
			var bbbccc = parseInt(strs1);
			zsdfjg1 =(zsdfjg1+bbbccc)/jzdhgl.length;
//			zsdfjg1 =eval(zsdfjg1+strs1);
		}
		var jzdfjg = document.getElementById('jzdfjg');
		var zsdfjg = document.getElementById('zsdfjg');
		jzdfjg.value = jzdfjg1;
		zsdfjg.value = zsdfjg1;
		var zdfjg =(jzdfjg1+zsdfjg1)/2;
		var gczdf = document.getElementById('gczdf');
		gczdf.value = zdfjg;
	</script>
</html>
